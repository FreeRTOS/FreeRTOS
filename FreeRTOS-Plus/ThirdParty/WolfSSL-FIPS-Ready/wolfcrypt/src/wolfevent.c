...
#include <wolfssl/wolfcrypt/settings.h>
    ...
    int wolfEvent_Init( WOLF_EVENT * event,
    ...
    int wolfEvent_Poll( WOLF_EVENT * event,
    ...
    int wolfEventQueue_Push( WOLF_EVENT_QUEUE * queue,
                             WOLF_EVENT * event )
    {
        int ret;
    ...
    int wolfEventQueue_Pop( WOLF_EVENT_QUEUE * queue,
                            WOLF_EVENT ** event )
    {
        int ret = 0;
...
/* assumes queue is locked by caller */
    int wolfEventQueue_Add( WOLF_EVENT_QUEUE * queue,
                            WOLF_EVENT * event )
    {
        if( ( queue == NULL ) || ( event == NULL ) )
        {
...
/* assumes queue is locked by caller */
    int wolfEventQueue_Remove( WOLF_EVENT_QUEUE * queue,
                               WOLF_EVENT * event )
    {
        int ret = 0;
    ...
    int wolfEventQueue_Poll( WOLF_EVENT_QUEUE * queue,
                             void * context_filter,
                             WOLF_EVENT ** events,
                             int maxEvents,
                             WOLF_EVENT_FLAG flags,
