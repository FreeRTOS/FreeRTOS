...
extern void pdma_transfer( uint64_t destination,
                           uint64_t source,
 ...
 */
    ...
    static void dumpbyte( mss_uart_instance_t * uart,
                          uint8_t b )
    {
        #ifdef DEBUG_DDR_INIT
            const uint8_t hexchrs[] =
            {
 ...
 */
    __attribute__( ( weak ) ) \
    void uprint32( mss_uart_instance_t * uart,
                   const char * msg,
                   uint32_t d )
    {
 ...
 */
    __attribute__( ( weak ) ) \
    void uprint64( mss_uart_instance_t * uart,
                   const char * msg,
                   uint64_t d )
    {
        MSS_UART_polled_tx_string( uart, ( const uint8_t * ) msg );
 ...
 */
    __attribute__( ( weak ) ) \
    void uprint( mss_uart_instance_t * uart,
                 const char * msg )
    {
        MSS_UART_polled_tx_string( uart, ( const uint8_t * ) msg );
    }
 ...
 * @param no_of_regs
 */
    void print_reg_array( mss_uart_instance_t * uart,
 ...
 */
uint32_t ddr_read( volatile uint64_t * DDR_word_ptr,
                   uint32_t no_of_access,
                   uint8_t data_ptrn,
                   DDR_ACCESS_SIZE data_size )
{
    ...
    for( i = 0; i < ( no_of_access ); i++ )
    {
        ...
        if( ddr_data != DATA )
        {
                ...
                #endif /* ifdef DEBUG_DDR_RD_RW_FAIL */
            #endif /* ifdef DEBUG_DDR_INIT */
            err_cnt++;
        }
        else
 ...
 * @return
 */
uint32_t ddr_read_write_fn( uint64_t * DDR_word_ptr,
                            uint32_t no_access, \
                            uint32_t pattern )
{
 ...
 */
#ifdef DEBUG_DDR_INIT
    uint32_t error_status( mss_uart_instance_t * g_mss_uart_debug_pt,
                           uint32_t error )
    {
 ...
 */
void load_ddr_pattern( uint64_t base,
                       uint32_t size,
 ...
 * @param length
 */
void load_test_buffers( uint32_t * p_cached_ddr,
                        uint32_t * p_not_cached_ddr,
                        uint64_t length )
{
    ( void ) length;
 ...
 */
uint32_t test_ddr( uint32_t no_of_iterations,
                   uint32_t size )
{
