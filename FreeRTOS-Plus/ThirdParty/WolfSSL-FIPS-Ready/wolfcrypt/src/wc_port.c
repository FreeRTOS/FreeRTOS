...
/* Helpful function to load file into allocated buffer */
    int wc_FileLoad( const char * fname,
                     unsigned char ** buf,
                     size_t * bufLen,
                     void * heap )
    {
...
#endif /* if !defined( WOLFSSL_USER_MUTEX ) */

#ifdef USE_WOLF_STRTOK
/* String token (delim) search. If str is null use nextp. */
    char * wc_strtok( char * str,
                      const char * delim,
                      char ** nextp )
    {
        char * ret;
 ...
 */
    ...
    int wc_FreeMutex( wolfSSL_Mutex * m )
    {
        ...
        #else  /* if ( OS_CFG_MUTEX_DEL_EN == DEF_ENABLED ) */
            return 0;
        #endif /* if ( OS_CFG_MUTEX_DEL_EN == DEF_ENABLED ) */
    }
...
/****  uT-Kernel malloc/free ***/
    ...
    static const osMutexDef_t * CMSIS_mutex[] =
    {
        osMutex( wolfSSL_mt0 ),
...
/* system memory pool */
    ...
    void * nucleus_malloc( unsigned long size,
                           void * heap,
                           int type )
    {
        STATUS status;
