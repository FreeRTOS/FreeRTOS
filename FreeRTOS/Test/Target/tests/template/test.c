...
void test_Template(void) {
  ...
  xTaskCreate(prvEventSemaphoreTask, "Sem", configMINIMAL_STACK_SIZE, NULL,
  ...
  xTaskCreate(prvSDKSemaphoreUseTask, "SDK Seamphore Use",
              ...
              mainSDK_SEMAPHORE_USE_TASK_PRIORITY, NULL);

  /* Create the software timer as described in the comments at the top of
  this file. */
  xExampleSoftwareTimer = xTimerCreate(
      /* A text name, purely to help
      debugging. */
      (const char *)"Timer", mainSOFTWARE_TIMER_PERIOD_MS,
...
static void prvQueueSendTask(void *pvParameters) {
  ...
  while (ulCountOfItemsSentOnQueue < TEST_ITERATIONS) {
  ...
  TEST_ASSERT_EQUAL_INT(TEST_ITERATIONS, ulCountOfItemsSentOnQueue);
...
static void prvEventSemaphoreTask(void *pvParameters) {
  while (ulCountOfReceivedSemaphores < TEST_ITERATIONS) {
  ...
  TEST_ASSERT_EQUAL_INT(TEST_ITERATIONS, ulCountOfReceivedSemaphores);
...
static void validatorTask(void *pvParameters) {
  for (;;) {
    vTaskDelay(mainSOFTWARE_TIMER_PERIOD_MS);
...
static void prvSDKSemaphoreUseTask(void *pvParameters) {
  ...
  TEST_ASSERT_EQUAL_INT(TEST_ITERATIONS, ulCountOfSDKSemaphoreAcquires);
