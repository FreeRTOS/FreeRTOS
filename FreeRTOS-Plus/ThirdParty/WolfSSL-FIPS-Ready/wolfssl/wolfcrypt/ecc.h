...
/* Determine max ECC bits based on enabled curves */
        ...
        #endif /* if defined( HAVE_ECC521 ) || defined( HAVE_ALL_CURVES ) */
...
/* ECC predefined curve sets  */
        ...
        const char * wc_ecc_get_name( int curve_id );
            ...
            ECC_API int ecc_map( ecc_point *,
                                 mp_int *,
                                 mp_digit );
            ECC_API int ecc_map_ex( ecc_point *,
                                    mp_int *,
                                    mp_digit,
                                    int ct );
...
/* ASN key helpers */
            WOLFSSL_API
            int wc_ecc_export_x963( ecc_key *,
                                    byte * out,
                                    word32 * outLen );
            WOLFSSL_API
            int wc_ecc_export_x963_ex( ecc_key *,
                                       byte * out,
                                       word32 * outLen,
                                       int compressed );
...
/* ecc encrypt */
            ...
            WOLFSSL_API
            int wc_ecc_ctx_reset( ecEncCtx *,
                                  WC_RNG * ); /* reset for use again w/o alloc/free */

            WOLFSSL_API
            const byte * wc_ecc_ctx_get_own_salt( ecEncCtx * );
            WOLFSSL_API
            int wc_ecc_ctx_set_peer_salt( ecEncCtx *,
                                          const byte * salt );
            WOLFSSL_API
            int wc_ecc_ctx_set_info( ecEncCtx *,
                                     const byte * info,
                                     int sz );
        ...
        #ifdef HAVE_X963_KDF
            WOLFSSL_API int wc_X963_KDF( enum wc_HashType type,
                                         const byte * secret,
                                         word32 secretSz,
                                         const byte * sinfo,
                                         word32 sinfoSz,
        ...
        #ifdef WOLFSSL_DSP
            WOLFSSL_API
            int wc_ecc_set_handle( ecc_key * key,
                                   remote_handle64 handle );
            WOLFSSL_LOCAL
            int sp_dsp_ecc_verify_256( remote_handle64 handle,
                                       const byte * hash,
                                       word32 hashLen,
                                       mp_int * pX,
                                       mp_int * pY,
                                       ...
                                       int * res,
                                       void * heap );
        #endif /* ifdef WOLFSSL_DSP */

        #ifdef WC_ECC_NONBLOCK
            WOLFSSL_API int wc_ecc_set_nonblock( ecc_key * key,
                                                 ecc_nb_ctx_t * ctx );
        #endif
