...
/*    #endif */
    #endif /* if ( USE_EXT_STATIC_MEM == YES ) */
...
/* EMC_IO	   (PUP_CLEAR | SLEWRATE_FAST | INBUF_ENABLE | FILTER_DISABLE) */
    ...
    #endif /* if ( USE_EXT_DYNAMIC_MEM == YES ) */
}
...
void EMC_Config_Static( void )
{
    ...
    #endif /* if ( PLATFORM == HITEX_A2_BOARD ) */
}
...
#endif /* if ( PLATFORM == HITEX_A2_BOARD ) || ( PLATFORM == NXP_VALIDATION_BOARD ) */
...
*
****************************************************************************************/
void EMC_Init_SRDRAM( uint32_t u32BaseAddr,
                      uint32_t u32Width,
                      uint32_t u32Size,
                      uint32_t u32DataBus,
                      uint32_t u32ColAddrBits )
{
