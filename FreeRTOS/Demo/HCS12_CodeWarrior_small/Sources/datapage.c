...
#endif /* if defined( HCS12 ) || defined( _HCS12 ) */
...
#endif /* if !defined( __DPAGE__ ) && !defined( __EPAGE__ ) && !defined( __PPAGE__ ) */
...
L_DPAGE:
                ...
                #endif
                CPY  # DPAGE_HIGH_BOUND;
                test of higher bound DPAGE / lower bound PPAGE
...
FOUND_DPAGE:
                LDX  # DPAGE_ADDR;
                load page register address and clear zero flag
                    RTS

                #if defined( __PPAGE__ )
L_PPAGE:
                    CPY  # PPAGE_HIGH_BOUND;
                    test of higher bound of PPAGE
                    BHI L_NOPAGE
FOUND_PPAGE:
                    LDX  # PPAGE_ADDR;
                    load page register address and clear zero flag
...
L_EPAGE:
                    CPY # EPAGE_LOW_BOUND;
                    test of lower bound of EPAGE
                    BLO L_NOPAGE
                        CPY # EPAGE_HIGH_BOUND;
                    test of higher bound of EPAGE
                    BHI L_NOPAGE
...
void NEAR _SET_PAGE( void )
{
    ...
    asm {
        PSHX;
        save X register __PIC_JSR( _GET_PAGE_REG )
        BEQ L_NOPAGE
        STAB   0, X;
        set page register
L_NOPAGE:
...
loop:
        ...
        calcutate actual source address
            LDAB    2, SP;
        load source page __PIC_JSR( _LOAD_FAR_8 );
        load 1 source byte
        PSHB;
        save value
            LDD     0 + 1, SP;
        ...
        restore value
            LDAB    3, SP;
        load destination page __PIC_JSR( _STORE_FAR_8 );
        store one byte
        DEX
        BNE loop
            LDX     6, SP;
