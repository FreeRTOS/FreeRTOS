...
NextZeroOut: PSHY;
        save nofZeroOuts
            ...
            load page of destination address
                LDY   2, X +;
            load offset of destination address __PIC_JSR( _SET_PAGE );
...
NextWord:   CLR   1, Y +;
            clear memory byte __FEED_COP_IN_HLI();
            feed the COP

            if necessary                                                /*lint !e505 !e522 asm code */
            DBNE D, NextWord;
            dec byte count
        #else
            LSRD;
...
LoopClrW:   STX   2, Y +;
            ...
            handle last byte
                CLR   1, Y +
LastClr:
        #endif /* ifdef  __OPTIMIZE_FOR_SIZE__ */
        PULY;
        restore nofZeroOuts
            DEY;
        dec nofZeroOuts
...
NextBlock:
        ...
        BEQ funcInits;
            ...
            load destination page
                LDY   2, X +;
            destination address __PIC_JSR( _SET_PAGE );
        ...
        #ifdef  __OPTIMIZE_FOR_SIZE__       /* -os, default */
Copy:       MOVB  1, X +, 1, Y +;
            move a byte from ROM to the data area __FEED_COP_IN_HLI();
            feed the COP
            ...
            / 2 and save bit 0 in the carry
Copy:       MOVW  2, X +, 2, Y +;
            move a word from ROM to the data area __FEED_COP_IN_HLI();
            feed the COP
...
#endif /* if defined( __ELF_OBJECT_FILE_FORMAT__ ) && defined( __cplusplus ) */
