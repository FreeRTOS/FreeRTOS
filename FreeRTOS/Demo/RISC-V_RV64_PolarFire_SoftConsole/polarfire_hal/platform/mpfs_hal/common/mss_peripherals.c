 ...
 * @return
 */
static inline uint8_t verify_context_enable( uint8_t option,
                                             uint32_t periph_context_mask,
                                             uint32_t hart )
{
    uint8_t result = 1U;
    ...
    #else  /* if ( ( LIBERO_SETTING_MEM_CONFIGS_ENABLED & PMP_ENABLED_MASK ) == PMP_ENABLED_MASK ) */
        ( void ) hart;
        ( void ) periph_context_mask;
        ( void ) option;
        result = 0U;
    #endif /* if ( ( LIBERO_SETTING_MEM_CONFIGS_ENABLED & PMP_ENABLED_MASK ) == PMP_ENABLED_MASK ) */
    return result;
}
 ...
 * @param req_state
 */
static inline void peripheral_on_off( uint32_t peripheral_mask,
                                      PERIPH_RESET_STATE req_state )
{
 ...
 */
__attribute__( ( weak ) ) uint8_t mss_config_clk_rst( mss_peripherals peripheral,
                                                      uint8_t hart,
                                                      PERIPH_RESET_STATE req_state )
{
    uint8_t result = 1U;
