...
/* Initialize CRL Entry */
        static int InitCRL_Entry( CRL_Entry * crle,
                                  DecodedCRL * dcrl,
                                  const byte * buff,
                                  int verified,
                                  void * heap )
...
/* Free all CRL resources */
        ...
        static int CheckCertCRLList( WOLFSSL_CRL * crl,
                                     DecodedCert * cert,
                                     int * pFoundEntry )
        {
            CRL_Entry * crle;
...
/* Add Decoded CRL, 0 on success */
        static int AddCRL( WOLFSSL_CRL * crl,
                           DecodedCRL * dcrl,
                           const byte * buff,
                           int verified )
        {
...
/* Load CRL File of type, WOLFSSL_SUCCESS on ok */
                           ...
                           int verify )
        {
            ...
            if( type == WOLFSSL_FILETYPE_PEM )
            {
                ...
                #else  /* ifdef WOLFSSL_PEM_TO_DER */
                    ret = NOT_COMPILED_IN;
                #endif /* ifdef WOLFSSL_PEM_TO_DER */
            }
...
/* returns head of copied list that was alloc'd */
            static RevokedCert * DupRevokedCertList( RevokedCert * in,
                                                     void * heap )
            {
...
/* returns a deep copy of ent on success and null on fail */
            static CRL_Entry * DupCRL_Entry( const CRL_Entry * ent,
                                             void * heap )
            {
                CRL_Entry * dup;
...
/* returns the head of a deep copy of the list on success and null on fail */
            static CRL_Entry * DupCRL_list( CRL_Entry * crl,
                                            void * heap )
            {
                CRL_Entry * current;
                CRL_Entry * head = NULL;
 ...
 * return 0 on success */
            static int DupX509_CRL( WOLFSSL_X509_CRL * dup,
...
/* returns WOLFSSL_SUCCESS on success. Does not take ownership of newcrl */
            int wolfSSL_X509_STORE_add_crl( WOLFSSL_X509_STORE * store,
        ...
        #endif /* if defined( OPENSSL_EXTRA ) && defined( HAVE_CRL ) */

        #ifdef HAVE_CRL_MONITOR


/* Signal Monitor thread is setup, save status to setup flag, 0 on success */
            static int SignalSetup( WOLFSSL_CRL * crl,
                                    int status )
            {
                int ret;
...
/* OS X  monitoring */
                ...
                #ifndef max
                    static WC_INLINE int max( int a,
                                              int b )
                    {
                        return a > b ? a : b;
                    }
...
/* Load CRL path files of type, WOLFSSL_SUCCESS on ok */
        ...
        #else  /* if !defined( NO_FILESYSTEM ) && !defined( NO_WOLFSSL_DIR ) */
            int LoadCRL( WOLFSSL_CRL * crl,
                         const char * path,
                         int type,
                         int monitor )
