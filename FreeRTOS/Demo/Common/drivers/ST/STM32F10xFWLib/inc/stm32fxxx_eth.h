...
#define IS_ETH_JUMBO_FRAME( CMD )             \
    ( ( ( CMD ) == ETH_JumboFrame_Enable ) || \
...
#define IS_ETH_CHECKSUM_OFFLOAD( CMD )             \
    ( ( ( CMD ) == ETH_ChecksumOffload_Enable ) || \
...
#define IS_ETH_PROMISCUOUS_MODE( CMD )             \
    ( ( ( CMD ) == ETH_PromiscuousMode_Enable ) || \
...
#define IS_ETH_DMA_TXDESC_SEGMENT( SEGMENT )          \
    ( ( ( SEGMENT ) == ETH_DMATxDesc_LastSegment ) || \
...
#define IS_ETH_RECEIVE_THRESHOLD_CONTROL( THRESHOLD )             \
    ( ( ( THRESHOLD ) == ETH_ReceiveThresholdControl_64Bytes ) || \
...
#define IS_ETH_SECOND_FRAME_OPERATE( CMD )            \
    ( ( ( CMD ) == ETH_SecondFrameOperate_Enable ) || \
...
#define IS_ETH_MMC_IT( IT )                                                                           \
    ( ( ( ( ( IT ) &( u32 ) 0xFFDF3FFF ) == 0x00 ) || ( ( ( IT ) &( u32 ) 0xEFFDFF9F ) == 0x00 ) ) && \
      ( ( IT ) != 0x00 ) )
...
#define IS_ETH_PTP_UPDATE( UPDATE )           \
    ( ( ( UPDATE ) == ETH_PTP_FineUpdate ) || \
...
#define IS_ETH_PTP_TIME_SIGN( SIGN )          \
...
#define IS_ETH_PTP_REGISTER( REG )                                  \
    ( ( ( REG ) == ETH_PTPTSCR ) || ( ( REG ) == ETH_PTPSSIR ) ||   \
