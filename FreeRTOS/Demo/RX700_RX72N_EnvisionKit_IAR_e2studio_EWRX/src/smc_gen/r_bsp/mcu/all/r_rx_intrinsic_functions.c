...
***********************************************************************************************************************/
#if defined( __GNUC__ )
    signed long R_BSP_Max( signed long data1,
                           signed long data2 )
    {
...
***********************************************************************************************************************/
#if defined( __GNUC__ )
    signed long R_BSP_Min( signed long data1,
                           signed long data2 )
    {
        return ( data1 < data2 ) ? data1 : data2;
    }
...
***********************************************************************************************************************/
#if defined( __GNUC__ )
    unsigned long R_BSP_RotateLeft( unsigned long data,
                                    unsigned long num )
    {
...
***********************************************************************************************************************/
#if defined( __GNUC__ )
    unsigned long R_BSP_RotateRight( unsigned long data,
                                     unsigned long num )
    {
...
***********************************************************************************************************************/
    ...
    signed long long R_BSP_SignedMultiplication( signed long data1,
                                                 signed long data2 )
    {
...
***********************************************************************************************************************/
#if defined( __GNUC__ ) || defined( __ICCRX__ )
    unsigned long long R_BSP_UnsignedMultiplication( unsigned long data1,
                                                     unsigned long data2 )
    {
...
{
    R_BSP_ASM_BEGIN R_BSP_ASM(;
                              _R_BSP_Change_PSW_PM_to_UserMode : )
    R_BSP_ASM( PUSH.L R1;
               push the R1 value )
    R_BSP_ASM( MVFC PSW, R1;
               get the current PSW value )
    R_BSP_ASM( BTST    # 20, R1;
               check PSW.PM )
    R_BSP_ASM( BNE.B R_BSP_ASM_LAB_NEXT( 0 );
               _psw_pm_is_user_mode )
    R_BSP_ASM(;
              _psw_pm_is_supervisor_mode: )
    R_BSP_ASM( BSET    # 20, R1;
               change PM = 0 ( Supervisor Mode ) -- > 1 ( User Mode ) )
    R_BSP_ASM( PUSH.L R2;
               push the R2 value )
    R_BSP_ASM( MOV.L R0, R2;
               move the current SP value to the R2 value )
    R_BSP_ASM( XCHG    8[ R2 ].L, R1;
               exchange the value of R2 destination address and the R1 value )
    R_BSP_ASM(;
              ( exchange the return address value of caller and the PSW value ) )
    R_BSP_ASM( XCHG    4[ R2 ].L, R1;
               exchange the value of R2 destination address and the R1 value )
    R_BSP_ASM(;
              ( exchange the R1 value of stack and the return address value of caller ) )
    R_BSP_ASM( POP R2;
               pop the R2 value of stack )
    R_BSP_ASM( RTE )
    R_BSP_ASM_LAB( 0 :;
                   _psw_pm_is_user_mode: )
    R_BSP_ASM( POP R1;
               pop the R1 value of stack )
    R_BSP_ASM(;
              RTS )
    R_BSP_ASM_END
...
***********************************************************************************************************************/
#if defined( __GNUC__ )
    long R_BSP_MulAndAccOperation_2byte( short * data1,
                                         short * data2,
                                         unsigned long count )
    {
...
***********************************************************************************************************************/
#if defined( __GNUC__ )
    short R_BSP_MulAndAccOperation_FixedPoint1( short * data1,
                                                short * data2,
                                                unsigned long count )
    {
        register signed long * ldata1 = ( signed long * ) data1;
...
***********************************************************************************************************************/
#if defined( __GNUC__ )
    short R_BSP_MulAndAccOperation_FixedPoint2( short * data1,
                                                short * data2,
                                                unsigned long count )
    {
        register signed long * ldata1 = ( signed long * ) data1;
...
void R_BSP_SetBPSW( uint32_t data )
{
    ...
    R_BSP_ASM_BEGIN R_BSP_ASM( MVTC R1,
                               BPSW )
    R_BSP_ASM_END
...
void bsp_get_bpsw( uint32_t * ret )
{
    ...
    R_BSP_ASM_BEGIN R_BSP_ASM( PUSH.L R2 )
...
void R_BSP_MoveToAccHiLong( int32_t data )
{
    ...
    R_BSP_ASM_BEGIN R_BSP_ASM( MVTACHI R1 )
    R_BSP_ASM_END
...
void R_BSP_MoveToAccLoLong( int32_t data )
{
    ...
    R_BSP_ASM_BEGIN R_BSP_ASM( MVTACLO R1 )
    R_BSP_ASM_END
...
void R_BSP_BitSet( uint8_t * data,
                   uint32_t bit )
{
    R_BSP_ASM_INTERNAL_USED( data )
    R_BSP_ASM_INTERNAL_USED( bit )

    R_BSP_ASM_BEGIN R_BSP_ASM( BSET R2,
                               [ R1 ] )
    R_BSP_ASM_END
...
void R_BSP_BitClear( uint8_t * data,
                     uint32_t bit )
{
    R_BSP_ASM_INTERNAL_USED( data )
    R_BSP_ASM_INTERNAL_USED( bit )

    R_BSP_ASM_BEGIN R_BSP_ASM( BCLR R2,
                               [ R1 ] )
    R_BSP_ASM_END
...
void R_BSP_BitReverse( uint8_t * data,
                       uint32_t bit )
{
    R_BSP_ASM_INTERNAL_USED( data )
    R_BSP_ASM_INTERNAL_USED( bit )

    R_BSP_ASM_BEGIN R_BSP_ASM( BNOT R2,
                               [ R1 ] )
    R_BSP_ASM_END
...
***********************************************************************************************************************/
        ...
        void R_BSP_SetDPSW( uint32_t data )
        {
            ...
            R_BSP_ASM_BEGIN R_BSP_ASM( MVTDC R1,
                                       DPSW )
            R_BSP_ASM_END
...
***********************************************************************************************************************/
        ...
        {
            R_BSP_ASM_BEGIN R_BSP_ASM( PUSH.L R1 )
...
***********************************************************************************************************************/
            ...
            void R_BSP_CalcSine_Cosine( float f,
                                        float * sin,
                                        float * cos )
            {
                R_BSP_ASM_BEGIN R_BSP_ASM( PUSH.L R4 )
