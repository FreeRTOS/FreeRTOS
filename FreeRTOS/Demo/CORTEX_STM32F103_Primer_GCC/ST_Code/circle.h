...
/*typedef void  (*tHandler)    ( void ); */
    ...
    void UTIL_SetSchHandler( enum eSchHandler,
                             tHandler );
...
/* Exported types ------------------------------------------------------------*/
    enum LED_mode
    {
        LED_UNDEF = -1, LED_OFF = 0, LED_ON = 1, LED_BLINKING_LF = 2, LED_BLINKING_HF = 3
    };
    enum LED_id
    {
...
/* Exported types ------------------------------------------------------------*/
    enum BUTTON_mode
    {
        BUTTON_DISABLED = -1, BUTTON_ONOFF = 0,
        BUTTON_ONOFF_FORMAIN = 1, BUTTON_WITHCLICK = 2
    };
    enum BUTTON_state
    {
        BUTTON_UNDEF = -1, BUTTON_RELEASED = 0, BUTTON_PUSHED = 1,
...
/* Exported types ------------------------------------------------------------*/
    enum POINTER_mode
    {
        POINTER_UNDEF = -1, POINTER_OFF = 0, POINTER_ON = 1, POINTER_MENU = 2, POINTER_APPLICATION = 3, POINTER_RESTORE_LESS = 4
    };
    enum POINTER_state
    {
        POINTER_S_UNDEF = -1, POINTER_S_DISABLED = 0, POINTER_S_ENABLED = 1
    };
...
/* Exported functions --------------------------------------------------------*/
    ...
    void POINTER_SetCurrentPointer( unsigned char width,
                                    unsigned char height,
                                    unsigned char * bmp );
    enum POINTER_state POINTER_GetState( void );
    void POINTER_Draw( u8 Line,
                       u8 Column,
                       u8 Width,
                       u8 Height,
                       u8 * Bmp );
    void POINTER_SetRect( s16 x,
                          s16 y,
                          s16 width,
                          s16 height ); /*Restrict the move of the pointer to a rectangle */
    void POINTER_SetRectScreen( void ); /*Remove any space restriction for the pointer moves. */
    void POINTER_Save( u8 Line,
                       u8 Column,
                       u8 Width,
                       u8 Height );
    void POINTER_Restore( u8 Line,
                          u8 Column,
                          u8 Width,
                          u8 Height );
...
/* Exported type def ---------------------------------------------------------*/
    enum  BUZZER_mode
    {
        BUZZER_UNDEF = -1, BUZZER_OFF = 0, BUZZER_ON = 1,
...
/* Exported type functions ---------------------------------------------------*/
    void RTC_Init( void );
    void RTC_SetTime( u32 THH,
                      u32 TMM,
                      u32 TSS );
    void RTC_GetTime( u32 * THH,
                      u32 * TMM,
                      u32 * TSS );
