 ...
 *
 */
uint16_t LFXT_Start_Timeout( uint16_t xtdrive,
                             uint16_t timeout )
{
    UCSCTL6_L |= XT1DRIVE1_L + XT1DRIVE0_L;                       /* Highest drive setting for XT1 startup */
 ...
 *
 */
uint16_t XT1_Start_Timeout( uint16_t xtdrive,
                            uint16_t timeout )
{
    UCSCTL6 &= ~( XT1OFF | XT1DRIVE_3 );                          /* enable XT1 */
    UCSCTL6 |= ( XTS | xtdrive );                                 /* enable XT1 and set XT1Drive */
 ...
 *
 */
uint16_t XT2_Start_Timeout( uint16_t xtdrive,
                            uint16_t timeout )
{
 ...
 */
void Init_FLL_Settle( uint16_t fsystem,
                      uint16_t ratio )
{
