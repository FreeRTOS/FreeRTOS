...
/* If building for old FIPS. */
    ...
    int wc_GenerateSeed( OS_Seed * os,
                         byte * seed,
                         word32 sz )
    {
                             ...
                             byte * b )
    {
        ...
        #include <wolfssl/wolfcrypt/sha256.h>
        ...
        #else  /* if defined( WOLFSSL_SGX ) */
            /* include headers that may be needed to get good seed */
            #include <fcntl.h>
            #ifndef EBSNET
                #include <unistd.h>
            #endif
        #endif /* if defined( WOLFSSL_SGX ) */
            ...
            static void wc_InitRng_IntelRD( void )
            {
                ...
                static int wc_GenerateSeed_IntelRD( OS_Seed * os,
                                                    byte * output,
                                                    word32 sz );
            #endif
            #ifdef HAVE_INTEL_RDRAND
                static int wc_GenerateRand_IntelRD( OS_Seed * os,
                                                    byte * output,
                                                    word32 sz );
            #endif
        ...
        #endif /* if defined( HAVE_INTEL_RDRAND ) || defined( HAVE_INTEL_RDSEED ) */
 ...
 * More is better, but more is also slower. */
                ...
                #endif
            #endif /* ifndef RNG_SECURITY_STRENGTH */
...
/* Returns: DRBG_SUCCESS or DRBG_FAILURE */
                                ...
                                word32 inBSz )
            {
                ...
                for( i = 0, ctr = 1; i < len; i++, ctr++ )
                {
                        ...
                        if( ret == 0 )
                    #endif /* ifndef WOLFSSL_SMALL_STACK_CACHE */
...
/* Returns: DRBG_SUCCESS or DRBG_FAILURE */
            static int Hash_DRBG_Reseed( DRBG * drbg,
                                         const byte * seed,
                                         word32 seedSz )
            {
...
/* Returns: DRBG_SUCCESS and DRBG_FAILURE or BAD_FUNC_ARG on fail */
            int wc_RNG_DRBG_Reseed( WC_RNG * rng,
                                    const byte * seed,
                                    word32 seedSz )
            {
            ...
            static WC_INLINE void array_add_one( byte * data,
                                                 word32 dataSz )
            {
                int i;
...
/* Returns: DRBG_SUCCESS or DRBG_FAILURE */
            ...
            static WC_INLINE void array_add( byte * d,
                                             word32 dLen,
                                             const byte * s,
                                             word32 sLen )
            {
...
/* Returns: DRBG_SUCCESS, DRBG_NEED_RESEED, or DRBG_FAILURE */
            static int Hash_DRBG_Generate( DRBG * drbg,
                                           byte * out,
                                           word32 outSz )
            {
                int ret;
...
/* Returns: DRBG_SUCCESS or DRBG_FAILURE */
            static int Hash_DRBG_Instantiate( DRBG * drbg,
                                              const byte * seed,
                                              word32 seedSz,
                                              const byte * nonce,
                                              word32 nonceSz,
...
/* Returns: DRBG_SUCCESS or DRBG_FAILURE */
            ...
            int wc_RNG_TestSeed( const byte * seed,
                                 word32 seedSz )
            {
                int ret = 0;
...
/* End NIST DRBG Code */
        ...
        static int _InitRng( WC_RNG * rng,
                             byte * nonce,
                             word32 nonceSz,
                             void * heap,
                             int devId )
            ...
            /* configure async RNG source if available */
                ...
                ret = 0; /* success */
                    ...
                    if( wc_RNG_HealthTestLocal( 0 ) == 0 )
                    {
                        ...
                        #endif /* if !defined( WOLFSSL_NO_MALLOC ) || defined( WOLFSSL_STATIC_MEMORY ) */
        ...
        int wc_InitRngNonce( WC_RNG * rng,
                             byte * nonce,
                             word32 nonceSz )
        {
        ...
        int wc_InitRngNonce_ex( WC_RNG * rng,
                                byte * nonce,
                                word32 nonceSz,
                                void * heap,
                                int devId )
...
/* place a generated block in output */
                                  ...
                                  word32 sz )
        {
            ...
            #endif /* if defined( WOLFSSL_ASYNC_CRYPT ) */
 ...
 * #define CUSTOM_RAND_GENERATE  rand_gen  */
        ...
        !defined( WOLFSSL_PIC32MZ_RNG )
                                 ...
                                 word32 sz )
            {
            ...
            #endif /* ifdef MICROCHIP_MPLAB_HARMONY */
                                     ...
                                     word32 sz )
                {
                    ...
                    byte * op = output;
                        ...
                        RNGPOLY2 = RNGSEED2;
                        RNGPOLY1 = RNGSEED1;
                    #endif /* if ( ( __PIC32_FEATURE_SET0 == 'E' ) && ( __PIC32_FEATURE_SET1 == 'C' ) ) */
 ...
 */
            ...
            #else  /* if defined( FREESCALE_K70_RNGA ) || defined( FREESCALE_RNGA ) */
 ...
 * Manual (Chapter 24) for STM32F4xx family. */
            ...
            #else  /* ifdef WOLFSSL_STM32_CUBEMX */
...
/* Generate a RNG seed using the STM32 Standard Peripheral Library */
            ...
            extern int getRandom( int sz,
                                  unsigned char * output );

            int GenerateSeed( OS_Seed * os,
                              byte * output,
                              word32 sz )
            {
 ...
 */
        ...
        #else  /* if defined( CUSTOM_RAND_GENERATE_SEED ) */
...
/* may block */
        ...
        #endif /* if defined( CUSTOM_RAND_GENERATE_SEED ) */
