 ...
 *  the index into pll_configs of a valid configuration */
    static int find_closest_config( long ref_hz,
                                    long rate )
    {
        int closest_index = -1;
...
/* Configure the PLL and wait for it to lock */
    static void configure_pll( __metal_io_u32 * pllcfg,
                               __metal_io_u32 * plloutdiv,
                               const struct pll_config_t * config )
    {
        ...
        #else  /* ifdef __METAL_DT_RISCV_CLINT0_HANDLE */
        #pragma message( \
        No handle for CLINT or CLIC found, PLL might race with lock signal !)
        #endif /* ifdef __METAL_DT_RISCV_CLINT0_HANDLE */
