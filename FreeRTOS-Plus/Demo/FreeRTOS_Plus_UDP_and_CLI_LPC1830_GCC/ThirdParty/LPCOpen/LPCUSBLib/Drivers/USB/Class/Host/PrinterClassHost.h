...
#define __PRINTER_CLASS_HOST_H__
	...
	/* Public Interface - May be used in end-application: */
		...
		/* Function Prototypes: */
			/** @brief Host interface configuration routine, to configure a given Printer host interface instance using the
			 ...
			 *
			 *  @param PRNTInterfaceInfo      : Pointer to a structure containing a Printer Class host configuration and state.
			...
			/** @brief General management task for a given Printer host class interface, required for the correct operation of
			...
			/** @brief Configures the printer to enable Bidirectional mode, if it is not already in this mode. This should be called
			...
			/** @brief Retrieves the status of the virtual Printer port's inbound status lines. The result can then be masked against the
			 *  \c PRNT_PORTSTATUS_* macros to determine the printer port's status.
			 *
			 *  @param PRNTInterfaceInfo : Pointer to a structure containing a Printer Class host configuration and state.
			 *  @param PortStatus        : Location where the retrieved port status should be stored.
			 *
			 *  @return A value from the @ref USB_Host_SendControlErrorCodes_t enum.
			 */
			...
			/** @brief Soft-resets the attached printer, readying it for new commands.
			 *
			 *  @param PRNTInterfaceInfo : Pointer to a structure containing a Printer Class host configuration and state.
			 *
			 *  @return A value from the @ref USB_Host_SendControlErrorCodes_t enum.
			...
			/** @brief Sends the given null terminated string to the attached printer's input endpoint.
			 *
			 *  @pre This function must only be called when the Host state machine is in the @ref HOST_STATE_Configured state or the
			 *       call will fail.
			 *
			...
			/** @brief Sends the given raw data stream to the attached printer's input endpoint. This should contain commands that the
			 ...
			 *
			 *  @param PRNTInterfaceInfo : Pointer to a structure containing a Printer Class host configuration and state.
			 *  @param Buffer            : Pointer to a buffer containing the raw command stream to send to the printer.
			 *  @param Length            : Size in bytes of the command stream to be sent.
			 *
			 *  @return A value from the @ref Pipe_Stream_RW_ErrorCodes_t enum.
			 */
			...
			/** @brief Determines the number of bytes received by the printer interface from the device, waiting to be read. This indicates the number
			...
			/** @brief Retrieves the attached printer device's ID string, formatted according to IEEE 1284. This string is sent as a
			 ...
			 *
			 *  @param PRNTInterfaceInfo : Pointer to a structure containing a Printer Class host configuration and state.
			 *  @param DeviceIDString    : Pointer to a buffer where the Device ID string should be stored, in ASCII format.
			 *  @param BufferSize        : Size in bytes of the buffer allocated for the Device ID string.
			 *
			 *  @return A value from the @ref Pipe_Stream_RW_ErrorCodes_t enum.
			 */
