...
/* Linker symbols to calculate LIM allocated size */
    ...
    int sifive_ccache0_set_way_mask( uint32_t master_id,
                                     uint64_t waymask )
    {
    ...
    void sifive_ccache0_set_pmevent_selector( uint32_t counter,
                                              uint64_t mask )
    {
    ...
    uint64_t sifive_ccache0_get_pmevent_counter( uint32_t counter )
    {
        ...
        #endif /* if METAL_SIFIVE_CCACHE0_PERFMON_COUNTERS > 0 */
        #if __riscv_xlen == 32
...
#endif /* ifdef METAL_SIFIVE_CCACHE0 */
