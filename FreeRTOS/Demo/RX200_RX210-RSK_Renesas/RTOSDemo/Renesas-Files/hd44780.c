...
static
void lcd_write( uint8_t reg, /* b0:command(0)/data(1), b2..1:E1(2)/E2(1)/both(0)(don't care on single controller), b3:write high nibble only(don't care on 8-bit bus) */
...
void lcd_locate( uint8_t row, /* Cursor row position (0.._LCD_ROWS-1) */
                 ...
                 )
{
    ...
    {
        if( ( _LCD_COLS >= 2 ) && ( row & 1 ) )
        {
            col += 0x40;
        }

        if( LCD_IF_SPLIT && ( col >= _LCD_COLS / 2 ) )
        {
            col += 0x40 - _LCD_COLS / 2;
        }

        if( LCD_IF_ALTROW && ( row & 2 ) )
        {
            col += _LCD_COLS;
        }
...
#endif /* if _USE_POINT && _USE_CGRAM */
