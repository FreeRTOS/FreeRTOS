...
/* AES-CTR */
        ...
        #endif /* ifdef WOLFSSL_AES_COUNTER */
...
/* Define AES implementation includes and functions */
                    ...
                    /* STM32F2/F4/F7/L4/L5/H7/WB55 hardware AES support for ECB, CBC, CTR and GCM modes */
                        ...
                        static int wc_AesEncrypt( Aes * aes,
                                                  const byte * inBlock,
                                                  byte * outBlock )
                        {
                            int ret = 0;
                            ...
                            static int wc_AesDecrypt( Aes * aes,
                                                      const byte * inBlock,
                                                      byte * outBlock )
                            {
                                int ret = 0;
 ...
 * NOTE: no support for AES-CTR/GCM/CCM/Direct */
                    ...
                    #endif /* if defined( FREESCALE_LTC_AES_GCM ) */
 ...
 * prefix between clang/gcc/llvm etc */
                    ...
                    static int AES_set_encrypt_key( const unsigned char * userKey,
                                                    const int bits,
                                                    Aes * aes )
                    {
                        int ret;
                ...
                defined( HAVE_AESCCM ) )
                    ...
                    static int AES_ECB_encrypt( Aes * aes,
                                                const byte * inBlock,
                                                byte * outBlock,
                                                int sz )
                    {
                        ...
                        static int AES_ECB_decrypt( Aes * aes,
                                                    const byte * inBlock,
                                                    byte * outBlock,
                                                    int sz )
                        {
                    ...
                    #endif /* if defined( HAVE_AES_DECRYPT ) */
                    ...
                    #endif
                #else  /* if defined( STM32_CRYPTO ) */

/* using wolfCrypt software implementation */
                    #define NEED_AES_TABLES
                #endif /* if defined( STM32_CRYPTO ) */
                    ...
                    static const FLASH_QUALIFIER word32 rcon[] =
                    {
                        ...
                        #endif /* HAVE_AES_DECRYPT */
                    #endif /* ifndef WOLFSSL_AES_SMALL_TABLES */
                    ...
                    #define GETBYTE( x, y )    ( word32 ) ( ( byte ) ( ( x ) >> ( 8 * ( y ) ) ) )
                        ...
                        static word32 col_mul( word32 t,
                                               int i2,
                                               int i3,
                                               int ia,
                                               int ib )
                        {
                            ...
                            static word32 inv_col_mul( word32 t,
                                                       int i9,
                                                       int ib,
                                                       int id,
                                                       int ie )
                        ...
                        #endif /* if defined( HAVE_AES_CBC ) || defined( WOLFSSL_AES_DIRECT ) */
                    #endif /* ifdef WOLFSSL_AES_SMALL_TABLES */
...
/* load 4 Te Tables into cache by cache line stride */
                        ...
                        #else  /* ifndef WOLFSSL_AES_SMALL_TABLES */
...
/* Software AES - ECB Encrypt */
                        static void wc_AesEncrypt( Aes * aes,
                                                   const byte * inBlock,
                                                   byte * outBlock )
                        {
                            word32 s0, s1, s2, s3;
                            ...
                            if( ( r > 7 ) || ( r == 0 ) )
                            {
                                ...
                                if( haveAESNI && aes->use_aesni )
                                {
                                    ...
                                    if( ( wolfssl_word ) inBlock % AESNI_ALIGN )
                                    {
                                            ...
                                            return;
                                        #else  /* ifndef NO_WOLFSSL_ALLOC_ALIGN */
                                            WOLFSSL_MSG( "AES-ECB encrypt with bad alignment" );
                                            return;
                                        #endif /* ifndef NO_WOLFSSL_ALLOC_ALIGN */
                                    }
...
/* Software AES - ECB Decrypt */
                            static void wc_AesDecrypt( Aes * aes,
                                                       const byte * inBlock,
                                                       byte * outBlock )
                            {
                                word32 s0, s1, s2, s3;
...
/* wc_AesSetKey */
                                      ...
                                      int dir )
                    {
                        if( AESBuffIn == NULL )
                        {
                            ...
                            #else  /* if defined( HAVE_THREADX ) */
                            #warning "Allocate non-Cache buffers"
                            #endif /* if defined( HAVE_THREADX ) */
                                      ...
                                      int dir )
                    {
                        ...
                        #endif /* ifdef FREESCALE_MMCAU_CLASSIC */
...
/* Software AES - SetKey */
                    static int wc_AesSetKeyLocal( Aes * aes,
                                                  const byte * userKey,
                                                  word32 keylen,
                                                  const byte * iv,
                                                  int dir )
                        ...
                        defined( NO_WOLFSSL_ESP32WROOM32_CRYPT_AES ) )
                            ...
                            switch( keylen )
                            {
                                ...
                                if( dir == AES_DECRYPTION )
                                {
                                    ...
                                    #endif /* if !defined( WOLFSSL_AES_SMALL_TABLES ) */
                                }
                            #else  /* if defined( HAVE_AES_DECRYPT ) */
                                ( void ) dir;
                            #endif /* HAVE_AES_DECRYPT */
                            ( void ) temp;
                        #endif /* NEED_AES_TABLES */
                                      ...
                                      int dir )
                    {
                        ...
                        #endif /* ifdef WOLFSSL_IMX6_CAAM_BLOB */
...
/* Allow direct access to one block encrypt */
                        ...
                        #endif /* HAVE_AES_DECRYPT */
                    #else  /* if defined( HAVE_COLDFIRE_SEC ) */
                        /* Allow direct access to one block encrypt */
                        void wc_AesEncryptDirect( Aes * aes,
                                                  byte * out,
                                                  const byte * in )
...
/* AES-CBC */
                    ...
                    #elif defined( HAVE_COLDFIRE_SEC )
                        static int wc_AesCbcCrypt( Aes * aes,
                                                   byte * po,
                                                   const byte * pi,
                                                   word32 sz,
                                                   word32 descHeader )
...
/* Software AES - CBC Encrypt */
                                              ...
                                              word32 sz )
                        {
                                                  ...
                                                  word32 sz )
                            {
                                ...
                                #endif /* if defined( WOLFSSL_ASYNC_CRYPT ) && defined( WC_ASYNC_ENABLE_AES ) */
...
/* AES-CTR */
                        ...
                        int wc_AesCtrEncryptBlock( Aes * aes,
                                                   byte * out,
                                                   const byte * in )
                        {
                            int ret = 0;
                    ...
                    #else  /* ifdef STM32_CRYPTO */

/* Use software based AES counter */
                        #define NEED_AES_CTR_SOFT
                    #endif /* ifdef STM32_CRYPTO */
 ...
 */
                ...
                static WC_INLINE void IncCtr( byte * ctr,
                                              word32 ctrSz )
                {
                    int i;
                    ...
                    /* implemented in wolfcrypt/src/port/devcrypt/devcrypto_aes.c */
                        ...
                        static WC_INLINE void FlattenSzInBits( byte * buf,
                                                               word32 sz )
                        {
                                         ...
                                         word32 len )
                    {
                        ...
                        return ret;
                            ...
                            XASM_LINK( "AES_GCM_encrypt" );
                                ...
                                void AES_GCM_encrypt_avx1( const unsigned char * in,
                                                           unsigned char * out,
                                                           const unsigned char * addt,
                                                           const unsigned char * ivec,
                                                           unsigned char * tag,
                                ...
                                XASM_LINK( "AES_GCM_encrypt_avx1" );
                                    ...
                                    void AES_GCM_encrypt_avx2( const unsigned char * in,
                                                               unsigned char * out,
                                                               const unsigned char * addt,
                                                               const unsigned char * ivec,
                                                               unsigned char * tag,
                            ...
                            #ifdef HAVE_AES_DECRYPT
                                void AES_GCM_decrypt( const unsigned char * in,
                                                      unsigned char * out,
                                                      const unsigned char * addt,
                                                      const unsigned char * ivec,
                                ...
                                XASM_LINK( "AES_GCM_decrypt" );
                                #ifdef HAVE_INTEL_AVX1
                                    void AES_GCM_decrypt_avx1( const unsigned char * in,
                                                               unsigned char * out,
                                                               const unsigned char * addt,
                                                               const unsigned char * ivec,
                                                               const unsigned char * tag,
                                    ...
                                    #ifdef HAVE_INTEL_AVX2
                                        void AES_GCM_decrypt_avx2( const unsigned char * in,
                                                                   unsigned char * out,
                                                                   const unsigned char * addt,
                                                                   const unsigned char * ivec,
                                                                   const unsigned char * tag,
                            ...
                            #define M128_INIT( x, y )             \
    { S( ( x ), 7 ), S( ( x ), 6 ), S( ( x ), 5 ), S( ( x ), 4 ), \
 ...
 * inline assembly. Intrinsic functions are used instead. */
    ...
    _mm_storeu_si128( &( ( __m128i * ) out )[ i * 8 + 7 ], tmp8 );
                            ...
                            static __m128i gfmul_sw( __m128i a,
                                                     __m128i b )
                            {
                            ...
                            static void gfmul_only( __m128i a,
                                                    __m128i b,
                                                    __m128i * r0,
                                                    __m128i * r1 )
                            {
                            ...
                            static __m128i ghash_red( __m128i r0,
                                                      __m128i r1 )
                            {
                                __m128i t2, t3;
                                __m128i t5, t6, t7;
                            ...
                            static __m128i gfmul_shifted( __m128i a,
                                                          __m128i b )
                            {
                            ...
                            #ifndef AES_GCM_AESNI_NO_UNROLL
                                static __m128i gfmul8( __m128i a1,
                                                       __m128i a2,
                                                       __m128i a3,
                                                       __m128i a4,
                                                       __m128i a5,
                            ...
                            #endif /* ifndef AES_GCM_AESNI_NO_UNROLL */
...
/* end defined(WORD64_AVAILABLE) && !defined(GCM_WORD32) */
                        ...
                        #else  /* ifdef FREESCALE_LTC_AES_GCM */

                            #ifdef STM32_CRYPTO_AES_GCM

/* this function supports inline encrypt */
                                static int wc_AesGcmEncrypt_STM32( Aes * aes,
                                                                   byte * out,
                                                                   const byte * in,
                                                                   word32 sz,
                                                                   const byte * iv,
                                                   ...
                                                   word32 authInSz )
                            {
                                ...
                                #endif /* ifdef WOLFSSL_PIC32MZ_CRYPT */
...
/* Software AES - GCM Encrypt */
                        ...
                        #endif /* ifdef FREESCALE_LTC_AES_GCM */
...
/* this function supports inline decrypt */
                                    static int wc_AesGcmDecrypt_STM32( Aes * aes,
                                                                       byte * out,
                                                                       const byte * in,
                                                                       word32 sz,
                                                                       const byte * iv,
                                                       ...
                                                       word32 authInSz )
                                {
                                    ...
                                    if( ConstantCompare( authTag, Tprime, authTagSz ) != 0 )
                                    {
                                        ...
                                        /* process remainder using partial handling */
                                    #endif /* if defined( WOLFSSL_PIC32MZ_CRYPT ) */
 ...
 * functions */
                    ...
                    int wc_AesGcmEncrypt_ex( Aes * aes,
                                             byte * out,
                                             const byte * in,
                                             word32 sz,
                                             byte * ivOut,
                    ...
                    int wc_GmacVerify( const byte * key,
                                       word32 keySz,
                                       const byte * iv,
                                       word32 ivSz,
                                       const byte * authIn,
...
/* return 0 on success */
                ...
                #else  /* ifdef WOLFSSL_ARMASM */

/* Software CCM */
                    static void roll_x( Aes * aes,
                                        const byte * in,
                                        word32 inSz,
                                        byte * out )
                    {
                    ...
                    static WC_INLINE void AesCcmCtrInc( byte * B,
                                                        word32 lenSz )
                    {
                        word32 i;
                    ...
                    #ifdef WOLFSSL_AESNI
                        static WC_INLINE void AesCcmCtrIncSet4( byte * B,
                                                                word32 lenSz )
                        {
                            word32 i;
                        ...
                        static WC_INLINE void AesCcmCtrInc4( byte * B,
                                                             word32 lenSz )
                        {
                            word32 i;
...
/* abstract functions that call lower level AESCCM functions */
                    ...
                    int wc_AesCcmEncrypt_ex( Aes * aes,
                                             byte * out,
                                             const byte * in,
                                             word32 sz,
                                             byte * ivOut,
...
/* Software AES - ECB */
            ...
            #else  /* if defined( WOLFSSL_IMX6_CAAM ) && !defined( NO_IMX6_CAAM_AES ) */
...
/* Software AES - ECB */
            ...
            #endif /* if defined( WOLFSSL_IMX6_CAAM ) && !defined( NO_IMX6_CAAM_AES ) */
        #endif /* HAVE_AES_ECB */
...
/* Software AES - CFB Encrypt */
            static int wc_AesFeedbackEncrypt( Aes * aes,
                                              byte * out,
                                              const byte * in,
                                              word32 sz,
                                              byte mode )
...
/* Software AES - CFB Decrypt */
                static int wc_AesFeedbackDecrypt( Aes * aes,
                                                  byte * out,
                                                  const byte * in,
                                                  word32 sz,
                                                  byte mode )
...
/* shift the whole AES_BLOCK_SIZE array left by 8 or 1 bits */
            static void shiftLeftArray( byte * ary,
                                        byte shift )
            {
                int i;
...
/* returns 0 on success and negative values on failure */
            static int wc_AesFeedbackCFB8( Aes * aes,
                                           byte * out,
                                           const byte * in,
                                           word32 sz,
                                           byte dir )
...
/* returns 0 on success and negative values on failure */
            static int wc_AesFeedbackCFB1( Aes * aes,
                                           byte * out,
                                           const byte * in,
                                           word32 sz,
                                           byte dir )
...
/* Initialize key wrap counter with value */
            static WC_INLINE void InitKeyWrapCounter( byte * inOutCtr,
                                                      word32 value )
            {
                int i;
                word32 bytes;
...
/* perform AES key wrap (RFC3394), return out sz on success, negative on err */
            int wc_AesKeyWrap( const byte * key,
                               word32 keySz,
                               const byte * in,
                               word32 inSz,
                               byte * out,
            ...
            int wc_AesKeyUnWrap( const byte * key,
                                 word32 keySz,
                                 const byte * in,
                                 word32 inSz,
                                 byte * out,
...
/* helper function for encrypting / decrypting full buffer at once */
                static int _AesXtsHelper( Aes * aes,
                                          byte * out,
                                          const byte * in,
                                          word32 sz,
                                          int dir )
...
/* Software AES - XTS Encrypt  */
            int wc_AesXtsEncrypt( XtsAes * xaes,
                                  byte * out,
                                  const byte * in,
                                  word32 sz,
                                  const byte * i,
...
/* Software AES - XTS Decrypt */
            int wc_AesXtsDecrypt( XtsAes * xaes,
                                  byte * out,
                                  const byte * in,
                                  word32 sz,
                                  const byte * i,
