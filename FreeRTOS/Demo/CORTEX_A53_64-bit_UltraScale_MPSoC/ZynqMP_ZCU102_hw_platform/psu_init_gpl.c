...
/*##################################################################### */
    ...
    *  higher to reduce the frequency of update requests, which can have a
    * small impact on the latency of the first read request when the uMCTL2 is
    * idle. Unit: 1024 clocks
    ...
    * expires dfi_ctrlupd_req is sent and traffic is blocked until the dfi_ctrlupd
    * _ackx is received. PHY can use this idle time to recalibrate the delay
    * lines to the DLLs. The DFI controller update is also used to reset PHY FI
    ...
    *  be greater than DFIUPD1.dfi_t_ctrlupd_interval_min_x1024. Unit: 1024
    * clocks
