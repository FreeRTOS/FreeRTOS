...
extern int32_t target_get_sha( int32_t idx,
                               uint32_t * base,
                               uint32_t * irq );
...
static int32_t sha_set_mode( sha_mode_e mode )
{
    ...
    #endif
#endif /* ifndef CONFIG_SHA_BLOCK_MODE */
...
static int32_t sha_input_data( uint32_t * data,
                               uint32_t length )
{
    uint8_t i;
    uint32_t tmp;
...
static int32_t sha_get_data( sha_handle_t handle,
                             uint32_t * data )
{
...
#ifdef CONFIG_SHA_SUPPORT_MUL_THREAD
    static int32_t sha_set_data( sha_handle_t handle,
                                 uint32_t * data )
    {
        ck_sha_priv_t * sha_priv = handle;
...
#endif /* ifdef CONFIG_SHA_SUPPORT_MUL_THREAD */

static inline void sha_reverse_order( uint8_t * pdata,
                                      int32_t length )
{
 ...
 */
sha_handle_t csi_sha_initialize( sha_handle_t handle,
                                 sha_event_cb_t cb_event )
{
    #ifdef CONFIG_SHA_SUPPORT_MUL_THREAD
        uint32_t base = 0u;
        uint32_t irq;
 ...
 * \return      error code
 */
int32_t csi_sha_starts( sha_handle_t handle,
                        void * context )
{
    SHA_NULL_PARAM_CHK( handle );
...
#endif
int32_t csi_sha_update( sha_handle_t handle,
                        void * context,
                        const void * input,
                        uint32_t len )
{
    ...
    #else  /* ifdef CONFIG_SHA_SUPPORT_MUL_THREAD */
...
int32_t csi_sha_finish( sha_handle_t handle,
                        void * context,
                        void * output )
{
    SHA_NULL_PARAM_CHK( handle );
