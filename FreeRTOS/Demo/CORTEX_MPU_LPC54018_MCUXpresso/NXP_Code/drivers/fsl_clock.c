...
static void pllFindSel( uint32_t M,
                        uint32_t * pSelP,
                        uint32_t * pSelI,
                        uint32_t * pSelR );
/* Get predivider (N) from PLL NDEC setting */
static uint32_t findPllPreDiv( uint32_t ctrlReg,
                               uint32_t nDecReg );
/* Get postdivider (P) from PLL PDEC setting */
static uint32_t findPllPostDiv( uint32_t ctrlReg,
                                uint32_t pDecReg );
/* Get multiplier (M) from PLL MDEC and BYPASS_FBDIV2 settings */
static uint32_t findPllMMult( uint32_t ctrlReg,
                              uint32_t mDecReg );
...
static uint32_t FindGreatestCommonDivisor( uint32_t m,
                                           uint32_t n );
/* Set PLL output based on desired output rate */
static pll_error_t CLOCK_GetPllConfig( uint32_t finHz,
                                       uint32_t foutHz,
                                       pll_setup_t * pSetup );
...
static const uint8_t wdtFreqLookup[ 32 ] =
{
    0,  8,  12, 15, 18, 20, 24, 26, 28, 30, 32, 34, 36, 38, 40, 41,
...
#endif /* if defined( FSL_FROHF_SETTING_API_ADDRESS_DETERMINE_BY_ROM_VERSION ) && ( FSL_FROHF_SETTING_API_ADDRESS_DETERMINE_BY_ROM_VERSION ) */
...
void CLOCK_SetupFROClocking( uint32_t froFreq )
{
    ...
    #else  /* if defined( FSL_FROHF_SETTING_API_ADDRESS_DETERMINE_BY_ROM_VERSION ) && ( FSL_FROHF_SETTING_API_ADDRESS_DETERMINE_BY_ROM_VERSION ) */
        froRomAddr = FSL_ROM_VERSION_0A_FRO_SETTING_ADDR;
 ...
 */
static pll_error_t CLOCK_GetPllConfigInternal( uint32_t finHz,
                                               uint32_t foutHz,
                                               pll_setup_t * pSetup )
{
