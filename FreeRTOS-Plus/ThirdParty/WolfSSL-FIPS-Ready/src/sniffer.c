...
/* Free overall Sniffer */
        void ssl_FreeSniffer( void )
        {
            ...
            static int HashUpdate( HsHashes * hash,
                                   const byte * input,
                                   int sz )
            {
                int ret = 0;
            ...
            static int HashCopy( HS_Hashes * d,
                                 HsHashes * s )
            {
                #ifndef NO_OLD_TLS
                    #ifndef NO_SHA
...
/* Password Setting Callback */
        static int SetPassword( char * passwd,
                                int sz,
                                int rw,
                                void * userdata )
        {
...
/* Show Set Server info for Trace */
        static void TraceSetServer( const char * srv,
                                    int port,
                                    const char * keyFile )
        {
            if( TraceOn )
...
/* Convert network byte order address into human readable */
        static const char * IpToS( int version,
                                   void * src,
                                   char * dst )
        {
...
/* Show sequence and payload length for Trace */
        static void TraceSequence( word32 seq,
                                   int len )
        {
            if( TraceOn )
            {
...
/* Show sequence and payload length for Trace */
        static void TraceAck( word32 ack,
                              word32 expected )
        {
            if( TraceOn )
            {
...
/* Show relative expected and relative received sequences */
        static void TraceRelativeSequence( word32 expected,
                                           word32 got )
        {
            if( TraceOn )
            {
...
/* Show client FIN capture */
        static void TraceClientFin( word32 finSeq,
                                    word32 relSeq )
        {
            if( TraceOn )
            {
...
/* Show server FIN capture */
        static void TraceServerFin( word32 finSeq,
                                    word32 relSeq )
        {
            if( TraceOn )
            {
...
/* Show bytes added to old SSL App data */
        static void TraceAddedData( int newBytes,
                                    int existingBytes )
        {
            if( TraceOn )
            {
...
/* Show value added to a named statistic. */
            static void TraceStat( const char * name,
                                   int add )
            {
                if( TraceOn )
                {
...
/* Compare IpAddrInfo structs */
        static WC_INLINE int MatchAddr( IpAddrInfo l,
                                        IpAddrInfo r )
        {
...
/* return 1 is true, 0 is false */
        ...
        #endif /* ifndef WOLFSSL_SNIFFER_WATCH */


/* Get SnifferServer from IP and Port */
        static SnifferServer * GetSnifferServer( IpInfo * ipInfo,
                                                 TcpInfo * tcpInfo )
        {
            SnifferServer * sniffer;
            ...
            #else  /* ifndef WOLFSSL_SNIFFER_WATCH */
                ( void ) ipInfo;
                ( void ) tcpInfo;
            #endif /* ifndef WOLFSSL_SNIFFER_WATCH */
...
/* Hash the Session Info, return hash row */
        static word32 SessionHash( IpInfo * ipInfo,
                                   TcpInfo * tcpInfo )
        {
            word32 hash = 1;
...
/* Get Existing SnifferSession from IP and Port */
        static SnifferSession * GetSnifferSession( IpInfo * ipInfo,
                                                   TcpInfo * tcpInfo )
        {
            SnifferSession * session;
            ...
            static int LoadKeyFile( byte ** keyBuf,
                                    word32 * keyBufSz,
                                    const char * keyFile,
                                    int keySz,
                                    int typeKey,
        ...
        #endif /* if defined( HAVE_SNI ) || defined( WOLFSSL_SNIFFER_WATCH ) */
        ...
        #endif /* ifdef WOLFSSL_SNIFFER_WATCH */


        static int SetNamedPrivateKey( const char * name,
                                       const char * address,
                                       int port,
                                       const char * keyFile,
                                       int keySz,
...
/* returns 0 on success, -1 on error */
        int ssl_SetPrivateKey( const char * address,
                               int port,
                               const char * keyFile,
                               int typeKey,
                               const char * password,
        ...
        int ssl_SetPrivateKeyBuffer( const char * address,
                                     int port,
                                     const char * keyBuf,
                                     int keySz,
                                     int typeKey,
...
/* returns 0 on success, -1 on error */
            int ssl_SetEphemeralKey( const char * address,
                                     int port,
                                     const char * keyFile,
                                     int typeKey,
                                     const char * password,
            ...
            int ssl_SetEphemeralKeyBuffer( const char * address,
                                           int port,
                                           const char * keyBuf,
                                           int keySz,
                                           int typeKey,
...
/* returns 0 on success, -1 on error */
        static int CheckIp6Hdr( Ip6Hdr * iphdr,
                                IpInfo * info,
                                int length,
                                char * error )
        {
...
/* returns 0 on success, -1 on error */
        static int CheckIpHdr( IpHdr * iphdr,
                               IpInfo * info,
                               int length,
                               char * error )
        {
...
/* returns 0 on success, -1 on error */
        static int CheckTcpHdr( TcpHdr * tcphdr,
                                TcpInfo * info,
                                char * error )
        {
            TraceTcp( tcphdr );
...
/* Decode Record Layer Header */
        static int GetRecordHeader( const byte * input,
 ...
 * SSLInfo is not provided. */
        static void CopySessionInfo( SnifferSession * session,
                                     SSLInfo * sslInfo )
        {
            if( NULL != sslInfo )
            {
...
/* Call the session connection start callback. */
        ...
        #ifdef SHOW_SECRETS
            static void PrintSecret( const char * desc,
                                     const byte * buf,
                                     int sz )
            {
                int i;
...
/* Process Keys */
        static int SetupKeys( const byte * input,
                              int * sslBytes,
                              SnifferSession * session,
                              char * error,
                              KeyShareInfo * ksInfo,
...
/* Process Client Key Exchange, static RSA  */
        static int ProcessClientKeyExchange( const byte * input,
                                             int * sslBytes,
                                             SnifferSession * session,
                                             char * error )
        {
        ...
        #ifdef WOLFSSL_TLS13
            static int ProcessKeyShare( KeyShareInfo * info,
                                        const byte * input,
                                        int len,
                                        word16 filter_group )
            {
            ...
            static int ProcessServerKeyShare( SnifferSession * session,
                                              const byte * input,
                                              int len,
                                              char * error )
            {
...
/* Process Session Ticket */
        static int ProcessSessionTicket( const byte * input,
                                         int * sslBytes,
                                         SnifferSession * session,
                                         char * error )
        {
...
/* Process Server Hello */
        static int ProcessServerHello( int msgSz,
                                       const byte * input,
                                       int * sslBytes,
                                       SnifferSession * session,
                                       char * error )
...
/* Process normal Client Hello */
        static int ProcessClientHello( const byte * input,
                                       int * sslBytes,
                                       SnifferSession * session,
                                       char * error )
        {
            ...
            *sslBytes -= SUITE_LEN;
            ...
            static int KeyWatchCall( SnifferSession * session,
                                     const byte * data,
                                     int dataSz,
                                     char * error )
            {
...
/* Process Certificate */
            static int ProcessCertificate( const byte * input,
                                           int * sslBytes,
                                           SnifferSession * session,
                                           char * error )
            {
...
/* Process Finished */
        static int ProcessFinished( const byte * input,
                                    int size,
                                    int * sslBytes,
                                    SnifferSession * session,
                                    char * error )
...
/* Process HandShake input */
        static int DoHandShake( const byte * input,
                                int * sslBytes,
                                SnifferSession * session,
                                char * error )
        {
            ...
            *sslBytes -= HANDSHAKE_HEADER_SZ;
            ...
            #endif /* ifdef HAVE_SECURE_RENEGOTIATION */
...
/* Decrypt input into plain output, 0 on success */
        static int Decrypt( WOLFSSL * ssl,
                            byte * output,
                            const byte * input,
                            word32 sz )
        {
...
/* Decrypt input message into output, adjust output steam if needed */
        static const byte * DecryptMessage( WOLFSSL * ssl,
                                            const byte * input,
                                            word32 sz,
                                            byte * output,
                                            int * error,
...
/* remove session from table, use rowHint if no info (means we have a lock) */
        static void RemoveSession( SnifferSession * session,
                                   IpInfo * ipInfo,
                                   TcpInfo * tcpInfo,
                                   word32 rowHint )
        {
...
/* Create a new Sniffer Session */
        static SnifferSession * CreateSession( IpInfo * ipInfo,
                                               TcpInfo * tcpInfo,
                                               char * error )
        {
...
/* Process Old Client Hello Input */
            static int DoOldHello( SnifferSession * session,
                                   const byte * sslFrame,
                                   int * rhSize,
                                   int * sslBytes,
                                   char * error )
...
/* returns 0 on success, -1 on error */
        static int CheckHeaders( IpInfo * ipInfo,
                                 TcpInfo * tcpInfo,
                                 const byte * packet,
                                 int length,
                                 const byte ** sslFrame,
...
/* returns 0 on success (continue), -1 on error, 1 on success (end) */
        static int CheckSession( IpInfo * ipInfo,
                                 TcpInfo * tcpInfo,
                                 int sslBytes,
                                 SnifferSession ** session,
                                 char * error )
...
/* Create a Packet Buffer from *begin - end, adjust new *begin and bytesLeft */
        static PacketBuffer * CreateBuffer( word32 * begin,
                                            word32 end,
                                            const byte * data,
                                            int * bytesLeft )
        {
...
/* returns 1 (end) on success, -1, on error */
        static int AddToReassembly( byte from,
                                    word32 seq,
                                    const byte * sslFrame,
                                    int sslBytes,
                                    SnifferSession * session,
...
/* returns 1 for success (end) */
        static int AddFinCapture( SnifferSession * session,
                                  word32 sequence )
        {
            if( session->flags.side == WOLFSSL_SERVER_END )
            {
...
/* returns 0 on success (continue), -1 on error, 1 on success (end) */
        static int AdjustSequence( TcpInfo * tcpInfo,
                                   SnifferSession * session,
                                   int * sslBytes,
                                   const byte ** sslFrame,
                                   char * error )
        ...
        static int FixSequence( TcpInfo * tcpInfo,
                                SnifferSession * session )
        {
            word32 * expected = ( session->flags.side == WOLFSSL_SERVER_END ) ?
 ...
 * return 0 ok, <0 on error */
        static int CheckAck( TcpInfo * tcpInfo,
                             SnifferSession * session )
        {
            if( tcpInfo->ack )
            {
...
/* returns 0 on success (continue), -1 on error, 1 on success (end) */
        static int CheckSequence( IpInfo * ipInfo,
                                  TcpInfo * tcpInfo,
                                  SnifferSession * session,
                                  int * sslBytes,
                                  const byte ** sslFrame,
...
/* returns 0 on success (continue), -1 on error, 1 on success (end) */
        static int CheckPreRecord( IpInfo * ipInfo,
                                   TcpInfo * tcpInfo,
                                   const byte ** sslFrame,
                                   SnifferSession ** session,
                                   int * sslBytes,
                                   ...
                                   char * error )
        {
            ...
            if( vChain != NULL )
            {
                    ...
                    *sslFrame = ssl->buffers.inputBuffer.buffer;
                    *end = *sslFrame + *sslBytes;
                #endif /* ifdef WOLFSSL_SNIFFER_CHAIN_INPUT */
                ( void ) chainSz;
            }
...
/* returns 1 for TRUE, 0 for FALSE */
        static int HaveMoreInput( SnifferSession * session,
                                  const byte ** sslFrame,
                                  int * sslBytes,
                                  const byte ** end,
                                  char * error )
...
/* return Number of bytes on success, 0 for no data yet, and -1 on error */
        static int ProcessMessage( const byte * sslFrame,
                                   SnifferSession * session,
                                   int sslBytes,
                                   byte ** data,
                                   const byte * end,
                        ...
                        if( ret == 0 )
                        {
                            ...
                            if( ret ) /* may be blank message */
                            {
                                ...
                                else
                                {
                                    ...
                                    #else  /* ifdef WOLFSSL_SNIFFER_STORE_DATA_CB */
                                        ( void ) ctx;
                                        SetError( NO_DATA_DEST_STR, error, session,
                                                  FATAL_ERROR_STATE );
                                        return -1;
                                    #endif /* ifdef WOLFSSL_SNIFFER_STORE_DATA_CB */
                                }
...
/* Return 0=normal, else = session removed */
        static int CheckFinCapture( IpInfo * ipInfo,
                                    TcpInfo * tcpInfo,
                                    SnifferSession * session )
        {
            int ret = 0;
 ...
 * return true if removed, 0 otherwise */
        static int RemoveFatalSession( IpInfo * ipInfo,
                                       TcpInfo * tcpInfo,
                                       SnifferSession * session,
                                       char * error )
        {
...
/* returns Number of bytes on success, 0 for no data yet, and -1 on error */
        static int ssl_DecodePacketInternal( const byte * packet,
                                             int length,
                                             void * vChain,
                                             word32 chainSz,
                                             byte ** data,
...
/* Also returns Session Info if available */
        int ssl_DecodePacketWithSessionInfo( const unsigned char * packet,
                                             int length,
                                             unsigned char ** data,
                                             SSLInfo * sslInfo,
                                             char * error )
...
/* Passes in an IP/TCP packet for decoding (ethernet/localhost frame) removed */
/* returns Number of bytes on success, 0 for no data yet, and -1 on error */
        int ssl_DecodePacket( const byte * packet,
                              int length,
                              byte ** data,
                              char * error )
        {
            ...
            int ssl_DecodePacketWithChain( void * vChain,
                                           word32 chainSz,
                                           byte ** data,
                                           char * error )
            {
        ...
        #endif /* if defined( WOLFSSL_SNIFFER_CHAIN_INPUT ) && defined( WOLFSSL_SNIFFER_STORE_DATA_CB ) */


/* Deallocator for the decoded data buffer. */
/* returns 0 on success, -1 on error */
        int ssl_FreeDecodeBuffer( byte ** data,
                                  char * error )
        {
...
/* returns 0 on success, -1 on error */
        int ssl_FreeZeroDecodeBuffer( byte ** data,
                                      int sz,
                                      char * error )
        {
            ( void ) error;
...
/* returns 0 on success, -1 on error */
        int ssl_Trace( const char * traceFile,
                       char * error )
        {
            if( traceFile )
            {
 ...
 * -1 means unlimited
 * returns 0 on success, -1 on error */
        int ssl_EnableRecovery( int onOff,
                                int maxMemory,
                                char * error )
        {
            ( void ) error;
            ...
            int ssl_GetSessionStats( unsigned int * active,
                                     unsigned int * total,
                                     unsigned int * peak,
                                     unsigned int * maxSessions,
                                     unsigned int * missedData,
                                     unsigned int * reassemblyMem,
                                     char * error )
            {
                int ret;
        ...
        #endif /* ifdef WOLFSSL_SESSION_STATS */
 ...
 * returns 0 on success, -1 on error */
            ...
            int ssl_SetWatchKeyCtx( void * ctx,
                                    char * error )
            {
                ( void ) error;
                WatchCbCtx = ctx;
            ...
            int ssl_SetWatchKey_buffer( void * vSniffer,
                                        const byte * key,
                                        word32 keySz,
                                        int keyType,
                                        char * error )
            ...
            int ssl_SetWatchKey_file( void * vSniffer,
                                      const char * keyFile,
                                      int keyType,
                                      const char * password,
                                      char * error )
