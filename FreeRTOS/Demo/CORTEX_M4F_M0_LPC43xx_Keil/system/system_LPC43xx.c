 ...
 * @brief  Setup a clock
 */
void SetClock( CLKBASE_Type target_clk,
               CLKSRC_Type src_clk,
               CLKDIV_Type div )
{
 ...
 */
void SetPL160M( CLKSRC_Type src_clk,
                uint32_t mult )
{
 ...
 *			msel+1 = feedback-divider value M   (1 to 2^15)
 */
void SetPLLUSB( CLKSRC_Type src_clk,
                uint8_t enable )
{
    if( !enable )
    {
...
void EnableSourceClk( CLKSRC_Type src_clk )
{
    ...
    if( src_clk == SRC_ENET_RX_CLK )
    {
        scu_pinmux( 0xC, 0, PlainEnable, FUNC3 );                           /* enet_rx_clk on PC_0 func 3 */
    }

    if( src_clk == SRC_ENET_TX_CLK )
    {
        scu_pinmux( 0x1, 19, PlainEnable, FUNC0 );                          /* enet_tx_clk on P1_19 func 0 */
    }
...
void DisableSourceClk( CLKSRC_Type src_clk )
{
    ...
    if( src_clk == SRC_ENET_RX_CLK )
    {
        scu_pinmux( 0xC, 0, PlainEnable, FUNC0 );                           /* nc on PC_0 func 0 */
    }

    if( src_clk == SRC_ENET_TX_CLK )
    {
        scu_pinmux( 0x1, 19, PlainEnable, FUNC2 );                          /* nc on P1_19 func 2 */
    }
