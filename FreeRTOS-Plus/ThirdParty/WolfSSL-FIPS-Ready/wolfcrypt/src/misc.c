 ...
 * i.e., _rotl and _rotr */
            ...
            WC_STATIC WC_INLINE word32 rotlFixed( word32 x,
                                                  word32 y )
            {
                return y ? _lrotl( x, y ) : x;
            }

            WC_STATIC WC_INLINE word32 rotrFixed( word32 x,
                                                  word32 y )
            {
...
/* This routine performs a byte swap of words array of a given count. */
            WC_STATIC WC_INLINE void ByteReverseWords( word32 * out,
                                                       const word32 * in,
                                                       word32 byteCount )
            {
            ...
            WC_STATIC WC_INLINE word64 rotlFixed64( word64 x,
                                                    word64 y )
            {
                return ( x << y ) | ( x >> ( sizeof( y ) * 8 - y ) );
            }
            ...
            WC_STATIC WC_INLINE word64 rotrFixed64( word64 x,
                                                    word64 y )
            {
                return ( x >> y ) | ( x << ( sizeof( y ) * 8 - y ) );
            }
            ...
            WC_STATIC WC_INLINE void ByteReverseWords64( word64 * out,
                                                         const word64 * in,
                                                         word32 byteCount )
            {
 ...
 * of wolfssl_words, placing the result in <*r>. */
            WC_STATIC WC_INLINE void XorWords( wolfssl_word * r,
                                               const wolfssl_word * a,
                                               word32 n )
            {
                word32 i;
 ...
 * counts, placing the result in <*buf>. */
            ...
            WC_STATIC WC_INLINE void xorbuf( void * buf,
                                             const void * mask,
                                             word32 count )
            {
        ...
        #endif /* ifndef WOLFSSL_NO_XOR_OPS */

        #ifndef WOLFSSL_NO_FORCE_ZERO

/* This routine fills the first len bytes of the memory area pointed by mem
 * with zeros. It ensures compiler optimizations doesn't skip it  */
            WC_STATIC WC_INLINE void ForceZero( const void * mem,
                                                word32 len )
            {
                volatile byte * z = ( volatile byte * ) mem;
                    ...
                    #endif /* ifndef WOLFSSL_UNALIGNED_64BIT_ACCESS */
                    ...
                    z = ( volatile byte * ) w;
                #endif /* if defined( WOLFSSL_X86_64_BUILD ) && defined( WORD64_AVAILABLE ) */
        ...
        #endif /* ifndef WOLFSSL_NO_FORCE_ZERO */


        #ifndef WOLFSSL_NO_CONST_CMP
/* check all length bytes for equality, return 0 on success */
            WC_STATIC WC_INLINE int ConstantCompare( const byte * a,
                                                     const byte * b,
                                                     int length )
            {
                int i;
        ...
        #endif /* ifndef WOLFSSL_NO_CONST_CMP */
            ...
            WC_STATIC WC_INLINE word32 min( word32 a,
                                            word32 b )
            {
                return a > b ? b : a;
            }
            ...
            #define WOLFSSL_HAVE_MAX
                ...
                #define max    max
            #endif
            WC_STATIC WC_INLINE word32 max( word32 a,
                                            word32 b )
            {
                return a > b ? a : b;
            }
...
/* converts a 32 bit integer to 24 bit */
            WC_STATIC WC_INLINE void c32to24( word32 in,
                                              word24 out )
            {
                out[ 0 ] = ( in >> 16 ) & 0xff;
                out[ 1 ] = ( in >> 8 ) & 0xff;
...
/* convert 16 bit integer to opaque */
            WC_STATIC WC_INLINE void c16toa( word16 wc_u16,
                                             byte * c )
            {
                c[ 0 ] = ( wc_u16 >> 8 ) & 0xff;
                c[ 1 ] = wc_u16 & 0xff;
...
/* convert 32 bit integer to opaque */
            WC_STATIC WC_INLINE void c32toa( word32 wc_u32,
                                             byte * c )
            {
                c[ 0 ] = ( wc_u32 >> 24 ) & 0xff;
                c[ 1 ] = ( wc_u32 >> 16 ) & 0xff;
        ...
        #endif /* ifndef WOLFSSL_NO_INT_ENCODE */

        #ifndef WOLFSSL_NO_INT_DECODE
/* convert a 24 bit integer into a 32 bit one */
            WC_STATIC WC_INLINE void c24to32( const word24 wc_u24,
                                              word32 * wc_u32 )
            {
...
/* convert opaque to 24 bit integer */
            WC_STATIC WC_INLINE void ato24( const byte * c,
                                            word32 * wc_u24 )
            {
...
/* convert opaque to 16 bit integer */
            WC_STATIC WC_INLINE void ato16( const byte * c,
                                            word16 * wc_u16 )
            {
...
/* convert opaque to 32 bit integer */
            WC_STATIC WC_INLINE void ato32( const byte * c,
                                            word32 * wc_u32 )
            {
        ...
        #endif /* ifndef WOLFSSL_NO_INT_DECODE */


        #ifndef WOLFSSL_NO_CT_OPS
/* Constant time - mask set when a > b. */
            WC_STATIC WC_INLINE byte ctMaskGT( int a,
                                               int b )
            {
...
/* Constant time - mask set when a >= b. */
            WC_STATIC WC_INLINE byte ctMaskGTE( int a,
                                                int b )
            {
...
/* Constant time - mask set when a >= b. */
            WC_STATIC WC_INLINE int ctMaskIntGTE( int a,
                                                  int b )
            {
                return ( ( ( word32 ) a - b ) >> 31 ) - 1;
            }
...
/* Constant time - mask set when a < b. */
            WC_STATIC WC_INLINE byte ctMaskLT( int a,
                                               int b )
            {
...
/* Constant time - mask set when a <= b. */
            WC_STATIC WC_INLINE byte ctMaskLTE( int a,
                                                int b )
            {
...
/* Constant time - mask set when a == b. */
            WC_STATIC WC_INLINE byte ctMaskEq( int a,
                                               int b )
            {
...
/* Constant time - sets 16 bit integer mask when a > b */
            WC_STATIC WC_INLINE word16 ctMask16GT( int a,
                                                   int b )
            {
...
/* Constant time - sets 16 bit integer mask when a < b. */
            WC_STATIC WC_INLINE word16 ctMask16LT( int a,
                                                   int b )
            {
                return ( ( ( word32 ) b - a - 1 ) >> 31 ) - 1;
            }
...
/* Constant time - sets 16 bit integer mask when a == b. */
            WC_STATIC WC_INLINE word16 ctMask16Eq( int a,
                                                   int b )
            {
...
/* Constant time - mask set when a != b. */
            WC_STATIC WC_INLINE byte ctMaskNotEq( int a,
                                                  int b )
            {
                return ctMaskGT( a, b ) | ctMaskLT( a, b );
            }
...
/* Constant time - select a when mask is set and b otherwise. */
            WC_STATIC WC_INLINE byte ctMaskSel( byte m,
                                                byte a,
                                                byte b )
            {
...
/* Constant time - select integer a when mask is set and integer b otherwise. */
            WC_STATIC WC_INLINE int ctMaskSelInt( byte m,
                                                  int a,
                                                  int b )
            {
...
/* Constant time - bit set when a <= b. */
            WC_STATIC WC_INLINE byte ctSetLTE( int a,
                                               int b )
            {
                return ( ( word32 ) a - b - 1 ) >> 31;
            }
        #endif /* ifndef WOLFSSL_NO_CT_OPS */
