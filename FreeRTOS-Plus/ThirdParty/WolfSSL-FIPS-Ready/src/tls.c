...
#include <wolfssl/wolfcrypt/settings.h>
    ...
    #ifdef HAVE_QSH
        static int TLSX_AddQSHKey( QSHKey ** list,
                                   QSHKey * key );
        static byte * TLSX_QSHKeyFind_Pub( QSHKey * qsh,
                                           word16 * pubLen,
                                           word16 name );
        #if defined( HAVE_NTRU )
            static int TLSX_CreateNtruKey( WOLFSSL * ssl,
                                           int type );
        #endif
    #endif /* HAVE_QSH */
    ...
    defined( HAVE_TLS_EXTENSIONS )
        static int TLSX_PopulateSupportedGroups( WOLFSSL * ssl,
                                                 TLSX ** extensions );
    #endif
...
/* Optional Pre-Master-Secret logging for Wireshark */
                ...
                #define HSHASH_SZ    FINISHED_SZ
                    ...
                    int tsip_generateMasterSecretEx( byte cipherSuiteFirst,
                                                     byte cipherSuite,
                ...
                #endif /* if ( WOLFSSL_RENESAS_TSIP_VER >= 109 ) */

                int tsip_useable( const WOLFSSL * ssl );
                int tsip_generateSeesionKey( WOLFSSL * ssl );
                int tsip_generateVerifyData( const byte * ms,
                                             const byte * side,
            ...
            int BuildTlsHandshakeHash( WOLFSSL * ssl,
                                       byte * hash,
                                       word32 * hashLen )
            {
                int ret = 0;
            ...
            int BuildTlsFinished( WOLFSSL * ssl,
                                  Hashes * hashes,
                                  const byte * sender )
            {
                int ret;
 ...
 */
            ...
            static int _DeriveTlsKeys( byte * key_dig,
                                       word32 key_dig_len,
                                       const byte * ms,
                                       word32 msLen,
                                       const byte * sr,
...
/* External facing wrapper so user can call as well, 0 on success */
            int wolfSSL_DeriveTlsKeys( byte * key_dig,
                                       word32 key_dig_len,
                                       const byte * ms,
                                       word32 msLen,
                                       const byte * sr,
            ...
            static int _MakeTlsMasterSecret( byte * ms,
                                             word32 msLen,
                                             const byte * pms,
                                             word32 pmsLen,
                                             const byte * cr,
...
/* External facing wrapper so user can call as well, 0 on success */
            int wolfSSL_MakeTlsMasterSecret( byte * ms,
                                             word32 msLen,
                                             const byte * pms,
                                             word32 pmsLen,
                                             const byte * cr,
                                             ...
                                             int hash_type )
            {
                ...
                static int _MakeTlsExtendedMasterSecret( byte * ms,
                                                         word32 msLen,
                                                         const byte * pms,
                                                         word32 pmsLen,
                                                         const byte * sHash,
...
/* External facing wrapper so user can call as well, 0 on success */
                int wolfSSL_MakeTlsExtendedMasterSecret( byte * ms,
                                                         word32 msLen,
                                                         const byte * pms,
                                                         word32 pmsLen,
                                                         const byte * sHash,
            ...
            int MakeTlsMasterSecret( WOLFSSL * ssl )
            {
                ...
                #endif /* HAVE_EXTENDED_MASTER */
                {
                    ...
                    #endif /* if defined( WOLFSSL_RENESAS_TSIP_TLS ) && !defined( NO_WOLFSSL_RENESAS_TSIP_TLS_SESSION ) */
 ...
 */
                    static int Hmac_HashUpdate( Hmac * hmac,
                                                const byte * data,
                                                word32 sz )
                    {
 ...
 * returns 0 on success, otherwise failure.
 */
                    static int Hmac_HashFinalRaw( Hmac * hmac,
                                                  unsigned char * hash )
                    {
                        int ret = BAD_FUNC_ARG;
 ...
 * returns 0 on success, otherwise failure.
 */
                    static int Hmac_OuterHash( Hmac * hmac,
                                               unsigned char * mac )
                    {
                        int ret = BAD_FUNC_ARG;
                        wc_HashAlg hash;
 ...
 * returns 0 on success, otherwise failure.
 */
                    static int Hmac_UpdateFinal_CT( Hmac * hmac,
                                                    byte * digest,
                                                    const byte * in,
                                                    word32 sz,
                                                    byte * header )
                ...
                #endif /* if !defined( WOLFSSL_NO_HASH_RAW ) && !defined( HAVE_FIPS ) && !defined( HAVE_SELFTEST ) */
 ...
 * returns 0 on success, otherwise failure.
 */
                    static int Hmac_UpdateFinal( Hmac * hmac,
                                                 byte * digest,
                                                 const byte * in,
                                                 word32 sz,
                                                 byte * header )
                ...
                #endif /* if defined( WOLFSSL_NO_HASH_RAW ) || defined( HAVE_FIPS ) || defined( HAVE_SELFTEST ) || defined( HAVE_BLAKE2 ) */

                int TLS_hmac( WOLFSSL * ssl,
                              byte * digest,
                              const byte * in,
                              word32 sz,
                              int padSz,
                              ...
                              int epochOrder )
                {
                    ...
                    if( ret == 0 )
                    {
                        ...
                        if( verify && ( padSz >= 0 ) )
                        {
                            ...
                            #else  /* if !defined( WOLFSSL_NO_HASH_RAW ) && !defined( HAVE_FIPS ) && !defined( HAVE_SELFTEST ) */
...
/** Creates a new extension. */
            static TLSX * TLSX_New( TLSX_Type type,
                                    void * data,
                                    void * heap )
            {
 ...
 */
            static int TLSX_Push( TLSX ** list,
                                  TLSX_Type type,
                                  void * data,
                                  void * heap )
            {
 ...
 * Checks for duplicate extensions, keeps the newest.
 */
                static int TLSX_Prepend( TLSX ** list,
                                         TLSX_Type type,
                                         void * data,
                                         void * heap )
                {
                ...
                int TLSX_CheckUnsupportedExtension( WOLFSSL * ssl,
                                                    TLSX_Type type );
...
/** Creates a new ALPN object, providing protocol name to use. */
                static ALPN * TLSX_ALPN_New( char * protocol_name,
                                             word16 protocol_nameSz,
                                             void * heap )
                {
                    ALPN * alpn;
...
/** Writes the ALPN objects of a list in a buffer. */
                static word16 TLSX_ALPN_Write( ALPN * list,
                                               byte * output )
                {
                    ALPN * alpn;
                    word16 length = 0;
...
/** Finds a protocol name in the provided ALPN list */
                static ALPN * TLSX_ALPN_Find( ALPN * list,
                                              char * protocol_name,
                                              word16 size )
                {
                    ALPN * alpn;
...
/** Set the ALPN matching client and server requirements */
                static int TLSX_SetALPN( TLSX ** extensions,
                                         const void * data,
                                         word16 size,
                                         void * heap )
                {
 ...
 * client and server requirements */
                static int TLSX_ALPN_ParseAndSet( WOLFSSL * ssl,
                                                  byte * input,
                                                  word16 length,
                                                  byte isRequest )
                {
...
/** Add a protocol name to the list of accepted usable ones */
                int TLSX_UseALPN( TLSX ** extensions,
                                  const void * data,
                                  word16 size,
                                  byte options,
                                  void * heap )
...
/** Get the protocol name set by the server */
                int TLSX_ALPN_GetRequest( TLSX * extensions,
                                          void ** data,
                                          word16 * dataSz )
                {
                    TLSX * extension;
...
/** Creates a new SNI object. */
                static SNI * TLSX_SNI_New( byte type,
                                           const void * data,
                                           word16 size,
                                           void * heap )
                {
...
/** Writes the SNI objects of a list in a buffer. */
                static word16 TLSX_SNI_Write( SNI * list,
                                              byte * output )
                {
                    SNI * sni;
                    word16 length = 0;
...
/** Finds a SNI object in the provided list. */
                static SNI * TLSX_SNI_Find( SNI * list,
                                            byte type )
                {
                    SNI * sni = list;
...
/** Sets the status of a SNI object. */
                static void TLSX_SNI_SetStatus( TLSX * extensions,
                                                byte type,
                                                byte status )
                {
...
/** Gets the status of a SNI object. */
                byte TLSX_SNI_Status( TLSX * extensions,
                                      byte type )
                {
                    TLSX * extension = TLSX_Find( extensions, TLSX_SERVER_NAME );
                    SNI * sni = TLSX_SNI_Find( extension ? ( SNI * ) extension->data : NULL, type );
...
/** Parses a buffer of SNI extensions. */
                static int TLSX_SNI_Parse( WOLFSSL * ssl,
                                           byte * input,
                                           word16 length,
                                           byte isRequest )
                {
                    ...
                    #else  /* ifndef NO_WOLFSSL_SERVER */
                        ( void ) input;
                    #endif /* ifndef NO_WOLFSSL_SERVER */

                    return 0;
                }

                static int TLSX_SNI_VerifyParse( WOLFSSL * ssl,
                                                 byte isRequest )
                {
                    ( void ) ssl;
                ...
                int TLSX_UseSNI( TLSX ** extensions,
                                 byte type,
                                 const void * data,
                                 word16 size,
                                 void * heap )
...
/** Tells the SNI requested by the client. */
                    word16 TLSX_SNI_GetRequest( TLSX * extensions,
                                                byte type,
                                                void ** data )
                    {
                        TLSX * extension = TLSX_Find( extensions, TLSX_SERVER_NAME );
...
/** Sets the options for a SNI object. */
                    void TLSX_SNI_SetOptions( TLSX * extensions,
                                              byte type,
                                              byte options )
                    {
                        TLSX * extension = TLSX_Find( extensions, TLSX_SERVER_NAME );
...
/** Retrieves a SNI request from a client hello buffer. */
                    int TLSX_SNI_GetFromBuffer( const byte * clientHello,
                                                word32 helloSz,
                                                byte type,
                                                byte * sni,
                                                word32 * inOutSz )
...
/** Creates a new TCA object. */
                static TCA * TLSX_TCA_New( byte type,
                                           const byte * id,
                                           word16 idSz,
                                           void * heap )
                {
...
/** Releases a TCA object. */
                static void TLSX_TCA_Free( TCA * tca,
                                           void * heap )
                {
                    ( void ) heap;
...
/** Releases all TCA objects in the provided list. */
                static void TLSX_TCA_FreeAll( TCA * list,
                                              void * heap )
                {
                    TCA * tca;
...
/** Writes the TCA objects of a list in a buffer. */
                static word16 TLSX_TCA_Write( TCA * list,
                                              byte * output )
                {
                    TCA * tca;
                    word16 offset = OPAQUE16_LEN; /* list length offset */
                ...
                #ifndef NO_WOLFSSL_SERVER
                    static TCA * TLSX_TCA_Find( TCA * list,
                                                byte type,
                                                const byte * id,
                                                word16 idSz )
                    {
...
/** Parses a buffer of TCA extensions. */
                static int TLSX_TCA_Parse( WOLFSSL * ssl,
                                           const byte * input,
                                           word16 length,
                                           byte isRequest )
                {
...
/* Checks to see if the server sent a response for the TCA. */
                static int TLSX_TCA_VerifyParse( WOLFSSL * ssl,
                                                 byte isRequest )
                {
                    ( void ) ssl;
                ...
                int TLSX_UseTrustedCA( TLSX ** extensions,
                                       byte type,
                                       const byte * id,
                                       word16 idSz,
                                       void * heap )
...
/******************************************************************************/
                ...
                static word16 TLSX_MFL_Write( byte * data,
                                              byte * output )
                {
                    output[ 0 ] = data[ 0 ];
                ...
                static int TLSX_MFL_Parse( WOLFSSL * ssl,
                                           byte * input,
                                           word16 length,
                                           byte isRequest )
                {
                ...
                int TLSX_UseMaxFragment( TLSX ** extensions,
                                         byte mfl,
                                         void * heap )
                {
                    byte * data = NULL;
...
/******************************************************************************/
                ...
                static int TLSX_THM_Parse( WOLFSSL * ssl,
                                           byte * input,
                                           word16 length,
                                           byte isRequest )
                {
            ...
            #else  /* ifdef HAVE_TRUNCATED_HMAC */
...
/******************************************************************************/
                ...
                static word16 TLSX_CSR_GetSize( CertificateStatusRequest * csr,
                                                byte isRequest )
                {
                    word16 size = 0;
                ...
                static word16 TLSX_CSR_Write( CertificateStatusRequest * csr,
                                              byte * output,
                                              byte isRequest )
                {
                    /* shut up compiler warnings */
                    ...
                    #endif /* if defined( WOLFSSL_TLS13 ) && !defined( NO_WOLFSSL_SERVER ) */

                    return 0;
                }

                static int TLSX_CSR_Parse( WOLFSSL * ssl,
                                           byte * input,
                                           word16 length,
                                           byte isRequest )
                {
                ...
                int TLSX_CSR_InitRequest( TLSX * extensions,
                                          DecodedCert * cert,
                                          void * heap )
                {
                ...
                int TLSX_UseCertificateStatusRequest( TLSX ** extensions,
                                                      byte status_type,
                                                      byte options,
                                                      WOLFSSL * ssl,
                                                      void * heap,
            ...
            #else  /* ifdef HAVE_CERTIFICATE_STATUS_REQUEST */
...
/******************************************************************************/
                ...
                static int TLSX_CSR2_Parse( WOLFSSL * ssl,
                                            byte * input,
                                            word16 length,
                                            byte isRequest )
                {
                ...
                int TLSX_CSR2_InitRequests( TLSX * extensions,
                                            DecodedCert * cert,
                                            byte isPeer,
                                            void * heap )
                {
                ...
                void * TLSX_CSR2_GetRequest( TLSX * extensions,
                                             byte status_type,
                                             byte idx )
                {
                    TLSX * extension = TLSX_Find( extensions, TLSX_STATUS_REQUEST_V2 );
                ...
                int TLSX_UseCertificateStatusRequestV2( TLSX ** extensions,
                                                        byte status_type,
                                                        byte options,
                                                        void * heap,
                                                        int devId )
            ...
            #else  /* ifdef HAVE_CERTIFICATE_STATUS_REQUEST_V2 */
...
/******************************************************************************/
                ...
                static int TLSX_SupportedCurve_New( SupportedCurve ** curve,
                                                    word16 name,
                                                    void * heap )
                {
                    if( curve == NULL )
                ...
                static int TLSX_PointFormat_New( PointFormat ** point,
                                                 byte format,
                                                 void * heap )
                {
                    if( point == NULL )
                ...
                static void TLSX_SupportedCurve_FreeAll( SupportedCurve * list,
                                                         void * heap )
                {
                ...
                static void TLSX_PointFormat_FreeAll( PointFormat * list,
                                                      void * heap )
                {
                    PointFormat * point;
                ...
                static int TLSX_SupportedCurve_Append( SupportedCurve * list,
                                                       word16 name,
                                                       void * heap )
                {
                    int ret = BAD_FUNC_ARG;
                ...
                static int TLSX_PointFormat_Append( PointFormat * list,
                                                    byte format,
                                                    void * heap )
                {
                    int ret = BAD_FUNC_ARG;
                    ...
                    static void TLSX_SupportedCurve_ValidateRequest( WOLFSSL * ssl,
                                                                     byte * semaphore )
                    {
                        word16 i;
                    ...
                    static void TLSX_PointFormat_ValidateRequest( WOLFSSL * ssl,
                                                                  byte * semaphore )
                    {
                        word16 i;
                    ...
                    static void TLSX_PointFormat_ValidateResponse( WOLFSSL * ssl,
                                                                   byte * semaphore )
                    {
                ...
                static word16 TLSX_PointFormat_GetSize( PointFormat * list )
                {
                    ...
                    static word16 TLSX_SupportedCurve_Write( SupportedCurve * list,
                                                             byte * output )
                    {
                ...
                static word16 TLSX_PointFormat_Write( PointFormat * list,
                                                      byte * output )
                {
                ...
                !defined( WOLFSSL_NO_SERVER_GROUPS_EXT ) )
                    ...
                    static int TLSX_SupportedCurve_Parse( WOLFSSL * ssl,
                                                          byte * input,
                                                          word16 length,
                                                          byte isRequest )
                    {
                        ...
                        #endif /* if defined( WOLFSSL_TLS13 ) && !defined( WOLFSSL_NO_SERVER_GROUPS_EXT ) */
                ...
                #endif /* if !defined( NO_WOLFSSL_SERVER ) || ( defined( WOLFSSL_TLS13 ) && !defined( WOLFSSL_NO_SERVER_GROUPS_EXT ) ) */
 ...
 */
                    int TLSX_SupportedCurve_Preferred( WOLFSSL * ssl,
                                                       int checkSupported )
                    {
                        TLSX * extension;
                        SupportedCurve * curve;
                    ...
                    static int TLSX_PointFormat_Parse( WOLFSSL * ssl,
                                                       byte * input,
                                                       word16 length,
                                                       byte isRequest )
                    {
                    ...
                    #if defined( HAVE_ECC ) || defined( HAVE_CURVE25519 ) || defined( HAVE_CURVE448 )
                        int TLSX_ValidateSupportedCurves( WOLFSSL * ssl,
                                                          byte first,
                                                          byte second )
                        {
                            TLSX * extension = NULL;
                                 ...
                                 curve = curve->next )
                            {
                                ...
                                switch( curve->name )
                                {
                                        ...
                                        #endif /* if defined( HAVE_ECC160 ) || defined( HAVE_ALL_CURVES ) */
                                                ...
                                                case WOLFSSL_ECC_SECP192K1:
                                                    ...
                                                    break;
                                            #endif /* HAVE_ECC_KOBLITZ */
                                        #endif /* if defined( HAVE_ECC192 ) || defined( HAVE_ALL_CURVES ) */
                                                ...
                                                case WOLFSSL_ECC_SECP224K1:
                                                    ...
                                                    break;
                                            #endif /* HAVE_ECC_KOBLITZ */
                                        #endif /* if defined( HAVE_ECC224 ) || defined( HAVE_ALL_CURVES ) */
                                        ...
                                        #endif /* if !defined( NO_ECC256 ) || defined( HAVE_ALL_CURVES ) */
                                    #endif /* ifdef HAVE_ECC */
                                        ...
                                        case WOLFSSL_ECC_X448:
                                            ...
                                            break;
                                                ...
                                                case WOLFSSL_ECC_BRAINPOOLP384R1:
                                                    ...
                                                    break;
                                            #endif /* HAVE_ECC_BRAINPOOL */
                                        #endif /* if defined( HAVE_ECC384 ) || defined( HAVE_ALL_CURVES ) */
                    ...
                    #endif /* if defined( HAVE_ECC ) || defined( HAVE_CURVE25519 ) || defined( HAVE_CURVE448 ) */

                #endif /* NO_WOLFSSL_SERVER */

                int TLSX_UseSupportedCurve( TLSX ** extensions,
                                            word16 name,
                                            void * heap )
                {
                    TLSX * extension = NULL;
                ...
                int TLSX_UsePointFormat( TLSX ** extensions,
                                         byte format,
                                         void * heap )
                {
                    TLSX * extension = NULL;
            ...
            #else  /* ifdef HAVE_SUPPORTED_CURVES */
...
/******************************************************************************/
            ...
            || defined( HAVE_SERVER_RENEGOTIATION_INFO )
                ...
                static int TLSX_SecureRenegotiation_Parse( WOLFSSL * ssl,
                                                           byte * input,
                                                           word16 length,
                                                           byte isRequest )
                {
            ...
            #else  /* if defined( HAVE_SECURE_RENEGOTIATION ) || defined( HAVE_SERVER_RENEGOTIATION_INFO ) */
...
/******************************************************************************/
                ...
                static word16 TLSX_SessionTicket_GetSize( SessionTicket * ticket,
                                                          int isRequest )
                {
                    ( void ) isRequest;
                ...
                static word16 TLSX_SessionTicket_Write( SessionTicket * ticket,
                                                        byte * output,
                                                        int isRequest )
                {
                ...
                static int TLSX_SessionTicket_Parse( WOLFSSL * ssl,
                                                     byte * input,
                                                     word16 length,
                                                     byte isRequest )
                {
                ...
                int TLSX_UseSessionTicket( TLSX ** extensions,
            ...
            #else  /* ifdef HAVE_SESSION_TICKET */
...
/******************************************************************************/
                ...
                static int TLSX_QSH_Append( QSHScheme ** list,
                                            word16 name,
                                            byte * pub,
                                            word16 pubLen )
                {
...
/* request for server's public key : 02 indicates 0-2 requested */
                static byte TLSX_QSH_SerPKReq( byte * output,
                                               byte isRequest )
                {
                    if( isRequest )
                    {
...
/* check for TLS_QSH suite */
                    static void TLSX_QSH_ValidateRequest( WOLFSSL * ssl,
                                                          byte * semaphore )
                    {
                        int i;
 ...
 */
                    word16 TLSX_QSH_GetSize( QSHScheme * list,
                                             byte isRequest )
                    {
                        QSHScheme * temp = list;
                        word16 length = 0;
...
/* write out a list of QSHScheme IDs */
                    static word16 TLSX_QSH_Write( QSHScheme * list,
                                                  byte * output )
                    {
...
/* write public key list in extension */
                    static word16 TLSX_QSHPK_WriteR( QSHScheme * format,
                                                     byte * output )
                    {
                        word32 offset = 0;
                        word16 public_len = 0;
                    ...
                    word16 TLSX_QSHPK_Write( QSHScheme * list,
                                             byte * output )
                    {
                        QSHScheme * current = list;
                        word32 length = 0;
 ...
 */
                    static int TLSX_QSH_Parse( WOLFSSL * ssl,
                                               byte * input,
                                               word16 length,
                                               byte isRequest )
                    {
...
/* Used for parsing in QSHCipher structs on Key Exchange */
                    int TLSX_QSHCipher_Parse( WOLFSSL * ssl,
                                              const byte * input,
                                              word16 length,
                                              byte isServer )
                    {
...
/* return 1 on success */
                    int TLSX_ValidateQSHScheme( TLSX ** extensions,
                                                word16 theirs )
                    {
                        TLSX * extension = TLSX_Find( *extensions, TLSX_QUANTUM_SAFE_HYBRID );
                        QSHScheme * format = NULL;
...
/* Add a QSHScheme struct to list of usable ones */
                int TLSX_UseQSHScheme( TLSX ** extensions,
                                       word16 name,
                                       byte * pKey,
                                       word16 pkeySz,
                                       void * heap )
 ...
 *        0 otherwise.
 */
                    static int TLSX_EncryptThenMac_GetSize( byte msgType,
                                                            word16 * pSz )
                    {
                        ( void ) pSz;
 ...
 *        0 otherwise.
 */
                    static int TLSX_EncryptThenMac_Write( void * data,
                                                          byte * output,
                                                          byte msgType,
                                                          word16 * pSz )
                    {
 ...
 *        0 otherwise.
 */
                    static int TLSX_EncryptThenMac_Parse( WOLFSSL * ssl,
                                                          byte * input,
                                                          word16 length,
                                                          byte msgType )
                    {
 ...
 */
                static int TLSX_SupportedVersions_GetSize( void * data,
                                                           byte msgType,
                                                           word16 * pSz )
                {
 ...
 */
                static int TLSX_SupportedVersions_Write( void * data,
                                                         byte * output,
                                                         byte msgType,
                                                         word16 * pSz )
                {
 ...
 * returns 0 on success, otherwise failure.
 */
                static int TLSX_SupportedVersions_Parse( WOLFSSL * ssl,
                                                         byte * input,
                                                         word16 length,
                                                         byte msgType )
                {
 ...
 * returns 0 on success, otherwise failure.
 */
                static int TLSX_SetSupportedVersions( TLSX ** extensions,
                                                      const void * data,
                                                      void * heap )
                {
                    if( ( extensions == NULL ) || ( data == NULL ) )
 ...
 */
                static int TLSX_Cookie_GetSize( Cookie * cookie,
                                                byte msgType,
                                                word16 * pSz )
                {
 ...
 */
                static int TLSX_Cookie_Write( Cookie * cookie,
                                              byte * output,
                                              byte msgType,
                                              word16 * pSz )
                {
 ...
 */
                static int TLSX_Cookie_Parse( WOLFSSL * ssl,
                                              byte * input,
                                              word16 length,
                                              byte msgType )
                {
 ...
 * returns 0 on success and other values indicate failure.
 */
                int TLSX_Cookie_Use( WOLFSSL * ssl,
                                     byte * data,
                                     word16 len,
                                     byte * mac,
                                     byte macSz,
 ...
 */
                static int TLSX_SignatureAlgorithms_MapPss( WOLFSSL * ssl,
                                                            byte * input,
                                                            word16 length )
                {
                    word16 i;
 ...
 * returns the length of data that was written.
 */
                static word16 TLSX_SignatureAlgorithms_Write( void * data,
                                                              byte * output )
                {
                    WOLFSSL * ssl = ( WOLFSSL * ) data;
 ...
 * returns 0 on success, otherwise failure.
 */
                static int TLSX_SignatureAlgorithms_Parse( WOLFSSL * ssl,
                                                           byte * input,
                                                           word16 length,
                                                           byte isRequest,
                                                           Suites * suites )
 ...
 * returns 0 on success, otherwise failure.
 */
                static int TLSX_SetSignatureAlgorithms( TLSX ** extensions,
                                                        const void * data,
                                                        void * heap )
                {
                    if( extensions == NULL )
            ...
            #endif /* if !defined( WOLFSSL_NO_SIGALG ) */
/******************************************************************************/
 ...
 * returns the length of data that was written.
 */
                static word16 TLSX_SignatureAlgorithmsCert_Write( void * data,
                                                                  byte * output )
                {
                    WOLFSSL * ssl = ( WOLFSSL * ) data;
 ...
 * returns 0 on success, otherwise failure.
 */
                static int TLSX_SignatureAlgorithmsCert_Parse( WOLFSSL * ssl,
                                                               byte * input,
                                                               word16 length,
                                                               byte isRequest )
                {
 ...
 * returns 0 on success, otherwise failure.
 */
                static int TLSX_KeyShare_GenDhKey( WOLFSSL * ssl,
                                                   KeyShareEntry * kse )
                {
                    int ret;
                        ...
                        #endif /* if defined( WOLFSSL_STATIC_EPHEMERAL ) && defined( WOLFSSL_DH_EXTRA ) */
                        {
 ...
 * returns 0 on success, otherwise failure.
 */
                static int TLSX_KeyShare_GenX25519Key( WOLFSSL * ssl,
                                                       KeyShareEntry * kse )
                {
                    int ret;
 ...
 * returns 0 on success, otherwise failure.
 */
                static int TLSX_KeyShare_GenX448Key( WOLFSSL * ssl,
                                                     KeyShareEntry * kse )
                {
                    int ret;
 ...
 * returns 0 on success, otherwise failure.
 */
                static int TLSX_KeyShare_GenEccKey( WOLFSSL * ssl,
                                                    KeyShareEntry * kse )
                {
                    int ret;
 ...
 * heap  The heap used for allocation.
 */
                static void TLSX_KeyShare_FreeAll( KeyShareEntry * list,
                                                   void * heap )
                {
 ...
 */
                static word16 TLSX_KeyShare_GetSize( KeyShareEntry * list,
                                                     byte msgType )
                {
                    word16 len = 0;
 ...
 * returns the number of bytes written into the buffer.
 */
                static word16 TLSX_KeyShare_Write( KeyShareEntry * list,
                                                   byte * output,
                                                   byte msgType )
                {
                    word16 i = 0;
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_KeyShare_ProcessDh( WOLFSSL * ssl,
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_KeyShare_ProcessX448( WOLFSSL * ssl,
                                                      KeyShareEntry * keyShareEntry )
                {
                    int ret;
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_KeyShare_ProcessEcc( WOLFSSL * ssl,
                                                     KeyShareEntry * keyShareEntry )
                {
                    int ret;
 ...
 * number on error.
 */
                static int TLSX_KeyShareEntry_Parse( WOLFSSL * ssl,
                                                     byte * input,
                                                     word16 length,
                                                     KeyShareEntry ** kse )
                {
 ...
 */
                static int TLSX_KeyShare_Find( WOLFSSL * ssl,
                                               word16 group )
                {
                    TLSX * extension;
                    KeyShareEntry * list;
 ...
 * returns 1 when the extension has the group name and 0 otherwise.
 */
                static int TLSX_SupportedGroups_Find( WOLFSSL * ssl,
                                                      word16 name )
                {
                    #ifdef HAVE_SUPPORTED_CURVES
                        TLSX * extension;
                    ...
                    #endif /* ifdef HAVE_SUPPORTED_CURVES */
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_KeyShare_Parse( WOLFSSL * ssl,
                                                byte * input,
                                                word16 length,
                                                byte msgType )
                {
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_KeyShare_New( KeyShareEntry ** list,
                                              int group,
                                              void * heap,
                                              KeyShareEntry ** keyShareEntry )
                {
 ...
 * returns 0 on success and other values indicate failure.
 */
                int TLSX_KeyShare_Use( WOLFSSL * ssl,
                                       word16 group,
                                       word16 len,
                                       byte * data,
                                       KeyShareEntry ** kse )
 ...
 */
                static int TLSX_KeyShare_GroupRank( WOLFSSL * ssl,
                                                    int group )
                {
                    byte i;
                    ...
                    if( ssl->numGroups == 0 )
                    {
                                ...
                                #endif
                            #endif
                        #endif /* if defined( HAVE_ECC ) && defined( HAVE_SUPPORTED_CURVES ) */
 ...
 * heap  The heap used for allocation.
 */
                static void TLSX_PreSharedKey_FreeAll( PreSharedKey * list,
                                                       void * heap )
                {
 ...
 */
                static int TLSX_PreSharedKey_GetSize( PreSharedKey * list,
                                                      byte msgType,
                                                      word16 * pSz )
                {
                    if( msgType == client_hello )
 ...
 * SANITY_MSG_E to indicate invalid message type.
 */
                int TLSX_PreSharedKey_GetSizeBinders( PreSharedKey * list,
                                                      byte msgType,
                                                      word16 * pSz )
                {
                    word16 len;
 ...
 * returns the number of bytes written into the buffer.
 */
                int TLSX_PreSharedKey_WriteBinders( PreSharedKey * list,
                                                    byte * output,
                                                    byte msgType,
                                                    word16 * pSz )
                {
 ...
 * returns the number of bytes written into the buffer.
 */
                static int TLSX_PreSharedKey_Write( PreSharedKey * list,
                                                    byte * output,
                                                    byte msgType,
                                                    word16 * pSz )
                {
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_PreSharedKey_Parse( WOLFSSL * ssl,
                                                    byte * input,
                                                    word16 length,
                                                    byte msgType )
                {
                    ...
                    if( msgType == server_hello )
                    {
                        ...
                        #endif /* ifdef HAVE_SESSION_TICKET */
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_PreSharedKey_New( PreSharedKey ** list,
                                                  byte * identity,
                                                  word16 len,
                                                  void * heap,
 ...
 * returns 0 on success and other values indicate failure.
 */
                int TLSX_PreSharedKey_Use( WOLFSSL * ssl,
                                           byte * identity,
                                           word16 len,
                                           word32 age,
                                           byte hmac,
            ...
            #else  /* if defined( WOLFSSL_TLS13 ) && ( defined( HAVE_SESSION_TICKET ) || !defined( NO_PSK ) ) */
            ...
            #endif /* if defined( WOLFSSL_TLS13 ) && ( defined( HAVE_SESSION_TICKET ) || !defined( NO_PSK ) ) */
 ...
 */
                static int TLSX_PskKeModes_GetSize( byte modes,
                                                    byte msgType,
                                                    word16 * pSz )
                {
                    if( msgType == client_hello )
 ...
 * returns the number of bytes written into the buffer.
 */
                static int TLSX_PskKeModes_Write( byte modes,
                                                  byte * output,
                                                  byte msgType,
                                                  word16 * pSz )
                {
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_PskKeModes_Parse( WOLFSSL * ssl,
                                                  byte * input,
                                                  word16 length,
                                                  byte msgType )
                {
 ...
 * returns 0 on success and other values indicate failure.
 */
                int TLSX_PskKeModes_Use( WOLFSSL * ssl,
                                         byte modes )
                {
                    int ret = 0;
                    TLSX * extension;
 ...
 * extension.
 */
                static int TLSX_PostHandAuth_GetSize( byte msgType,
                                                      word16 * pSz )
                {
                    if( msgType == client_hello )
                    {
 ...
 * returns the number of bytes written into the buffer.
 */
                static int TLSX_PostHandAuth_Write( byte * output,
                                                    byte msgType,
                                                    word16 * pSz )
                {
                    ( void ) output;
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_PostHandAuth_Parse( WOLFSSL * ssl,
                                                    byte * input,
                                                    word16 length,
                                                    byte msgType )
                {
 ...
 */
            ...
            #else  /* if defined( WOLFSSL_TLS13 ) && defined( WOLFSSL_POST_HANDSHAKE_AUTH ) */

                #define PHA_GET_SIZE( a, b )       0
                #define PHA_WRITE( a, b, c )       0
                #define PHA_PARSE( a, b, c, d )    0

            #endif /* if defined( WOLFSSL_TLS13 ) && defined( WOLFSSL_POST_HANDSHAKE_AUTH ) */
 ...
 */
                static int TLSX_EarlyData_GetSize( byte msgType,
                                                   word16 * pSz )
                {
                    int ret = 0;
 ...
 * returns the number of bytes written into the buffer.
 */
                static int TLSX_EarlyData_Write( word32 max,
                                                 byte * output,
                                                 byte msgType,
                                                 word16 * pSz )
                {
 ...
 * returns 0 on success and other values indicate failure.
 */
                static int TLSX_EarlyData_Parse( WOLFSSL * ssl,
                                                 byte * input,
                                                 word16 length,
                                                 byte msgType )
                {
 ...
 * returns 0 on success and other values indicate failure.
 */
                int TLSX_EarlyData_Use( WOLFSSL * ssl,
                                        word32 max )
                {
                    int ret = 0;
                    TLSX * extension;
...
/** Tells the buffered size of the extensions in a list. */
            static int TLSX_GetSize( TLSX * list,
                                     byte * semaphore,
                                     byte msgType,
                                     word16 * pLength )
            {
...
/** Writes the extensions of a list in a buffer. */
            static int TLSX_Write( TLSX * list,
                                   byte * output,
                                   byte * semaphore,
                                   byte msgType,
                                   word16 * pOffset )
            {
                                    ...
                                    break;
                                        ...
                                        case TLSX_PSK_KEY_EXCHANGE_MODES:
                                            ...
                                            ret = PKM_WRITE( extension->val, output + offset, msgType,
                                                             &offset );
                                            break;
                                    #endif /* if defined( HAVE_SESSION_TICKET ) || !defined( NO_PSK ) */
                ...
                *pOffset += offset;
                ...
                static word32 GetEntropy( unsigned char * out,
                                          word32 num_bytes )
                {
                    int ret = 0;
            ...
            #endif /* if defined( HAVE_NTRU ) && defined( HAVE_QSH ) */


            #ifdef HAVE_QSH
                static int TLSX_CreateQSHKey( WOLFSSL * ssl,
                                              int type )
                {
                    int ret = -1;
                ...
                #if defined( HAVE_NTRU )
                    int TLSX_CreateNtruKey( WOLFSSL * ssl,
                                            int type )
                    {
                        int ret = -1;
                        int ntruType;
                    ...
                    if( !isServer && IsAtLeastTLSv1_3( ssl->version ) )
                    {
                        ...
                        if( TLSX_Find( ssl->extensions, TLSX_KEY_SHARE ) == NULL )
                        {
                            ...
                            #endif
                            {
                                ...
                                #else  /* if defined( HAVE_ECC ) && ( !defined( NO_ECC256 ) || defined( HAVE_ALL_CURVES ) ) && !defined( NO_ECC_SECP ) */
                                    return KEY_SHARE_ERROR;
                                #endif /* if defined( HAVE_ECC ) && ( !defined( NO_ECC256 ) || defined( HAVE_ALL_CURVES ) ) && !defined( NO_ECC_SECP ) */
                            }
                        ...
                        #endif /* if defined( HAVE_SESSION_TICKET ) */
                        #ifndef NO_PSK
...
/** Tells the buffered size of extensions to be sent into the client hello. */
                int TLSX_GetRequestSize( WOLFSSL * ssl,
                                         byte msgType,
                                         word16 * pLength )
                {
                    int ret = 0;
...
/** Writes the extensions to be sent into the client hello. */
                int TLSX_WriteRequest( WOLFSSL * ssl,
                                       byte * output,
                                       byte msgType,
                                       word16 * pOffset )
                {
...
/** Tells the buffered size of extensions to be sent into the server hello. */
                int TLSX_GetResponseSize( WOLFSSL * ssl,
                                          byte msgType,
                                          word16 * pLength )
                {
                    int ret = 0;
...
/** Writes the server hello extensions into a buffer. */
                int TLSX_WriteResponse( WOLFSSL * ssl,
                                        byte * output,
                                        byte msgType,
                                        word16 * pOffset )
                {
            ...
            #ifdef WOLFSSL_TLS13
                int TLSX_ParseVersion( WOLFSSL * ssl,
                                       byte * input,
                                       word16 length,
                                       byte msgType,
                                       int * found )
                        ...
                        case TLSX_STATUS_REQUEST_V2:
                            ...
                            break;
                                ...
                                case HELLO_EXT_EXTMS:
                                    ...
                                    pendingEMS = 1;
                                    break;
                            #endif /* ifdef HAVE_EXTENDED_MASTER */
