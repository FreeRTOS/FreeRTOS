...
#define IS_NVIC_IRQ_CHANNEL( CHANNEL )            \
    ( ( CHANNEL == WWDG_IRQChannel ) ||           \
...
#define IS_GET_PENDING_SYSTEM_HANDLER( HANDLER )   \
    ( ( HANDLER == SystemHandler_MemoryManage ) || \
      ( HANDLER == SystemHandler_BusFault ) ||     \
      ( HANDLER == SystemHandler_SVCall ) )

#define IS_SET_PENDING_SYSTEM_HANDLER( HANDLER ) \
    ( ( HANDLER == SystemHandler_NMI ) ||        \
      ( HANDLER == SystemHandler_PSV ) ||        \
      ( HANDLER == SystemHandler_SysTick ) )

#define IS_CLEAR_SYSTEM_HANDLER( HANDLER ) \
    ( ( HANDLER == SystemHandler_PSV ) ||  \
      ( HANDLER == SystemHandler_SysTick ) )

#define IS_GET_ACTIVE_SYSTEM_HANDLER( HANDLER )    \
    ( ( HANDLER == SystemHandler_MemoryManage ) || \
      ( HANDLER == SystemHandler_BusFault ) ||     \
      ( HANDLER == SystemHandler_UsageFault ) ||   \
      ( HANDLER == SystemHandler_SVCall ) ||       \
...
#define IS_FAULT_SOURCE_SYSTEM_HANDLER( HANDLER )  \
    ( ( HANDLER == SystemHandler_HardFault ) ||    \
      ( HANDLER == SystemHandler_MemoryManage ) || \
      ( HANDLER == SystemHandler_BusFault ) ||     \
      ( HANDLER == SystemHandler_UsageFault ) ||   \
      ( HANDLER == SystemHandler_DebugMonitor ) )

#define IS_FAULT_ADDRESS_SYSTEM_HANDLER( HANDLER ) \
    ( ( HANDLER == SystemHandler_MemoryManage ) || \
      ( HANDLER == SystemHandler_BusFault ) )
...
void NVIC_SetVectorTable( u32 NVIC_VectTab,
                          u32 Offset );
void NVIC_GenerateSystemReset( void );
void NVIC_GenerateCoreReset( void );
void NVIC_SystemLPConfig( u8 LowPowerMode,
                          FunctionalState NewState );
void NVIC_SystemHandlerConfig( u32 SystemHandler,
                               FunctionalState NewState );
