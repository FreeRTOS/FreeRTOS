...
#include <wolfssl/wolfcrypt/settings.h>
    ...
    #include <wolfssl/wolfcrypt/cpuid.h>
        ...
        static word32 cpu_flags_set = 0;
            ...
            #define ADD( out, in )        \
    { word64 t = out.lo; out.lo += in.lo; \
      out.hi += ( out.lo < t ) + in.hi; }
            #define ADDLO( out, in )   \
    { word64 t = out.lo; out.lo += in; \
      out.hi += ( out.lo < t ); }
    ...
    #endif /* if defined( USE_INTEL_SPEEDUP ) || defined( POLY130564 ) */
 ...
 */
            extern void poly1305_block_avx( Poly1305 * ctx,
 ...
 */
            extern void poly1305_blocks_avx( Poly1305 * ctx,
                                             const unsigned char * m,
                                             size_t bytes );
 ...
 */
            extern void poly1305_setkey_avx( Poly1305 * ctx,
                                             const byte * key );
 ...
 */
            extern void poly1305_final_avx( Poly1305 * ctx,
                                            byte * mac );
 ...
 * bytes  The number of bytes to process.
 */
            extern void poly1305_blocks_avx2( Poly1305 * ctx,
                                              const unsigned char * m,
                                              size_t bytes );
 ...
 * key  The key data (16 bytes).
 */
            extern void poly1305_setkey_avx2( Poly1305 * ctx,
                                              const byte * key );
 ...
 */
            extern void poly1305_final_avx2( Poly1305 * ctx,
                                             byte * mac );
...
/* convert 32-bit unsigned to little endian 64 bit type as byte array */
    static WC_INLINE void u32tole64( const word32 inLe32,
                                     byte outLe64[ 8 ] )
    {
 ...
 */
        static void poly1305_blocks( Poly1305 * ctx,
                                     const unsigned char * m,
                                     size_t bytes )
        {
            #ifdef USE_INTEL_SPEEDUP
 ...
 */
        static void poly1305_block( Poly1305 * ctx,
                                    const unsigned char * m )
        {
            #ifdef USE_INTEL_SPEEDUP
    ...
    #if !defined( WOLFSSL_ARMASM ) || !defined( __aarch64__ )
        int wc_Poly1305SetKey( Poly1305 * ctx,
                               const byte * key,
                               word32 keySz )
        {
        ...
        int wc_Poly1305Final( Poly1305 * ctx,
                              byte * mac )
        {
            #ifdef USE_INTEL_SPEEDUP
            #elif defined( POLY130564 )
    ...
    int wc_Poly1305Update( Poly1305 * ctx,
                           const byte * m,
                           word32 bytes )
    {
        size_t i;
 ...
 */
    int wc_Poly1305_Pad( Poly1305 * ctx,
                         word32 lenToPad )
    {
        int ret = 0;
        word32 paddingLen;
 ...
 */
    int wc_Poly1305_EncodeSizes( Poly1305 * ctx,
                                 word32 aadSz,
                                 word32 dataSz )
    {
        int ret;
