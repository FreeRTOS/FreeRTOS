 ...
 * the declspec tag. */
    ...
    #else  /* if defined( HAVE_THREAD_LS ) || defined( NO_THREAD_LS ) */
    ...
    #endif /* if defined( HAVE_THREAD_LS ) || defined( NO_THREAD_LS ) */
...
/* fips entry point, auto */
    ...
    #endif /* if defined( USE_WINDOWS_API ) && defined( WOLFSSL_DLL ) */
...
/* set iv wrapper */
        ...
        #ifdef HAVE_AES_ECB
/* ecb encrypt wrapper */
            int AesEcbEncrypt_fips( Aes * aes,
                                    byte * out,
                                    const byte * in,
                                    word32 sz )
            {
...
/* ecb decrypt wrapper */
            int AesEcbDecrypt_fips( Aes * aes,
                                    byte * out,
                                    const byte * in,
                                    word32 sz )
            {
        ...
        #ifdef HAVE_AES_CBC
/* cbc encrypt wrapper */
            int AesCbcEncrypt_fips( Aes * aes,
                                    byte * out,
                                    const byte * in,
                                    word32 sz )
            {
...
/* cbc decrypt wrapper */
            int AesCbcDecrypt_fips( Aes * aes,
                                    byte * out,
                                    const byte * in,
                                    word32 sz )
            {
...
/* ctr encrypt wrapper */
            int AesCtrEncrypt_fips( Aes * aes,
                                    byte * out,
                                    const byte * in,
                                    word32 sz )
            {
...
/* gcm set external iv wrapper */
        int AesGcmSetExtIV_fips( Aes * aes,
                                 const byte * iv,
                                 word32 ivSz )
        {
            if( FipsAllowed() != 0 )
...
/* gcm encrypt wrapper */
        int AesGcmEncrypt_fips( Aes * aes,
                                byte * out,
                                const byte * in,
                                word32 sz,
                                byte * ivOut,
...
/* gcm decrypt wrapper */
        int AesGcmDecrypt_fips( Aes * aes,
                                byte * out,
                                const byte * in,
                                word32 sz,
                                const byte * iv,
...
/* GMAC verify convenience wrapper */
        int GmacVerify_fips( const byte * key,
                             word32 keySz,
                             const byte * iv,
                             word32 ivSz,
                             const byte * authIn,
...
/* ccm set nonce wrapper */
        int AesCcmSetNonce_fips( Aes * aes,
                                 const byte * nonce,
                                 word32 nonceSz )
        {
            if( FipsAllowed() != 0 )
...
/* ccm encrypt wrapper */
        int AesCcmEncrypt_fips( Aes * aes,
                                byte * out,
                                const byte * in,
                                word32 sz,
                                byte * ivOut,
...
/* ccm decrypt wrapper */
        int AesCcmDecrypt_fips( Aes * aes,
                                byte * out,
                                const byte * in,
                                word32 sz,
                                const byte * iv,
...
/* cbc encrypt wrapper */
        int Des3_CbcEncrypt_fips( Des3 * des,
                                  byte * out,
                                  const byte * in,
                                  word32 sz )
        {
...
/* cbc decrypt wrapper */
        int Des3_CbcDecrypt_fips( Des3 * des,
                                  byte * out,
                                  const byte * in,
                                  word32 sz )
        {
...
/* SHA256 Update wrapper */
        int Sha256Update_fips( wc_Sha256 * sha,
                               const byte * data,
                               word32 len )
        {
            if( FipsAllowed() != 0 )
...
/* SHA256 Final wrapper */
        int Sha256Final_fips( wc_Sha256 * sha,
                              byte * hash )
        {
            if( FipsAllowed() != 0 )
            {
...
/* SHA224 Update wrapper */
            int Sha224Update_fips( wc_Sha224 * sha224,
                                   const byte * data,
                                   word32 len )
            {
                if( FipsAllowed() != 0 )
...
/* SHA224 Final wrapper */
            int Sha224Final_fips( wc_Sha224 * sha224,
                                  byte * hash )
            {
                if( FipsAllowed() != 0 )
                {
...
/* SHA512 Update wrapper */
        int Sha512Update_fips( wc_Sha512 * sha,
                               const byte * data,
                               word32 len )
        {
            if( FipsAllowed() != 0 )
...
/* SHA512 Final wrapper */
        int Sha512Final_fips( wc_Sha512 * sha,
                              byte * hash )
        {
            if( FipsAllowed() != 0 )
            {
...
/* SHA384 Update wrapper */
        int Sha384Update_fips( wc_Sha384 * sha,
                               const byte * data,
                               word32 len )
        {
            if( FipsAllowed() != 0 )
...
/* SHA384 Final wrapper */
        int Sha384Final_fips( wc_Sha384 * sha,
                              byte * hash )
        {
            if( FipsAllowed() != 0 )
            {
...
/* Base SHA-3 Functions */
        int InitSha3_224_fips( wc_Sha3 * sha3,
                               void * heap,
                               int devId )
        {
            if( FipsAllowed() != 0 )
        ...
        int Sha3_224_Update_fips( wc_Sha3 * sha3,
                                  const byte * data,
                                  word32 len )
        {
            if( FipsAllowed() != 0 )
        ...
        int Sha3_224_Final_fips( wc_Sha3 * sha3,
                                 byte * hash )
        {
            if( FipsAllowed() != 0 )
            {
        ...
        int InitSha3_256_fips( wc_Sha3 * sha3,
                               void * heap,
                               int devId )
        {
            if( FipsAllowed() != 0 )
        ...
        int Sha3_256_Update_fips( wc_Sha3 * sha3,
                                  const byte * data,
                                  word32 len )
        {
            if( FipsAllowed() != 0 )
        ...
        int Sha3_256_Final_fips( wc_Sha3 * sha3,
                                 byte * hash )
        {
            if( FipsAllowed() != 0 )
            {
        ...
        int InitSha3_384_fips( wc_Sha3 * sha3,
                               void * heap,
                               int devId )
        {
            if( FipsAllowed() != 0 )
        ...
        int Sha3_384_Update_fips( wc_Sha3 * sha3,
                                  const byte * data,
                                  word32 len )
        {
            if( FipsAllowed() != 0 )
        ...
        int Sha3_384_Final_fips( wc_Sha3 * sha3,
                                 byte * hash )
        {
            if( FipsAllowed() != 0 )
            {
        ...
        int InitSha3_512_fips( wc_Sha3 * sha3,
                               void * heap,
                               int devId )
        {
            if( FipsAllowed() != 0 )
        ...
        int Sha3_512_Update_fips( wc_Sha3 * sha3,
                                  const byte * data,
                                  word32 len )
        {
            if( FipsAllowed() != 0 )
        ...
        int Sha3_512_Final_fips( wc_Sha3 * sha3,
                                 byte * hash )
        {
            if( FipsAllowed() != 0 )
            {
...
/* RSA wrappers */
    #ifndef NO_RSA
/* Init RsaKey */
        int InitRsaKey_fips( RsaKey * key,
                             void * p )
        {
            if( FipsAllowed() != 0 )
            {
        ...
        int InitRsaKeyEx_fips( RsaKey * key,
                               void * p,
                               int devId )
        {
            if( FipsAllowed() != 0 )
...
/* Rsa Public Encrypt */
        int RsaPublicEncrypt_fips( const byte * in,
                                   word32 inLen,
                                   byte * out,
                                   word32 outLen,
                                   RsaKey * key,
...
/* Rsa Public Encrypt Extended */
        int RsaPublicEncryptEx_fips( const byte * in,
                                     word32 inLen,
                                     byte * out,
                                     word32 outLen,
                                     RsaKey * key,
...
/* Rsa Private Decrypt Inline */
        int RsaPrivateDecryptInline_fips( byte * in,
                                          word32 inLen,
                                          byte ** out,
                                          RsaKey * key )
        {
...
/* Rsa Private Decrypt Inline Extended */
        int RsaPrivateDecryptInlineEx_fips( byte * in,
                                            word32 inLen,
                                            byte ** out,
                                            RsaKey * key,
                                            int type,
...
/* Rsa Private Decrypt */
        int RsaPrivateDecrypt_fips( const byte * in,
                                    word32 inLen,
                                    byte * out,
                                    word32 outLen,
                                    RsaKey * key )
...
/* Rsa Private Decrypt Extended */
        int RsaPrivateDecryptEx_fips( const byte * in,
                                      word32 inLen,
                                      byte * out,
                                      word32 outLen,
                                      RsaKey * key,
...
/* Rsa SSL Sign */
        int RsaSSL_Sign_fips( const byte * in,
                              word32 inLen,
                              byte * out,
                              word32 outLen,
                              RsaKey * key,
...
/* Rsa SSL Verify Inline */
        int RsaSSL_VerifyInline_fips( byte * in,
                                      word32 inLen,
                                      byte ** out,
                                      RsaKey * key )
        {
...
/* Rsa SSL Verify */
        int RsaSSL_Verify_fips( const byte * in,
                                word32 inLen,
                                byte * out,
                                word32 outLen,
                                RsaKey * key )
        ...
        #ifdef WC_RSA_PSS
/* Rsa PSS Sign */
            int RsaPSS_Sign_fips( const byte * in,
                                  word32 inLen,
                                  byte * out,
                                  word32 outLen,
                                  enum wc_HashType hash,
...
/* Rsa PSS Sign Extended */
            int RsaPSS_SignEx_fips( const byte * in,
                                    word32 inLen,
                                    byte * out,
                                    word32 outLen,
                                    enum wc_HashType hash,
...
/* Rsa PSS Verify Inline */
            int RsaPSS_VerifyInline_fips( byte * in,
                                          word32 inLen,
                                          byte ** out,
                                          enum wc_HashType hash,
                                          int mgf,
...
/* Rsa PSS Verify Inline Extended */
            int RsaPSS_VerifyInlineEx_fips( byte * in,
                                            word32 inLen,
                                            byte ** out,
                                            enum wc_HashType hash,
                                            int mgf,
...
/* Rsa PSS Verify */
            int RsaPSS_Verify_fips( byte * in,
                                    word32 inLen,
                                    byte * out,
                                    word32 outLen,
                                    enum wc_HashType hash,
...
/* Rsa PSS Verify Extended */
            int RsaPSS_VerifyEx_fips( byte * in,
                                      word32 inLen,
                                      byte * out,
                                      word32 outLen,
                                      enum wc_HashType hash,
...
/* Rsa PSS Check Padding */
            int RsaPSS_CheckPadding_fips( const byte * in,
                                          word32 inSz,
                                          byte * sig,
                                          word32 sigSz,
...
/* Rsa PSS Check Padding Extended */
            int RsaPSS_CheckPaddingEx_fips( const byte * in,
                                            word32 inSz,
                                            byte * sig,
                                            word32 sigSz,
                                            enum wc_HashType hashType,
...
/* Rsa PrivateKey Decode */
            int RsaPrivateKeyDecode_fips( const byte * input,
                                          word32 * inOutIdx,
                                          RsaKey * key,
                                          word32 inSz )
            {
...
/* Rsa PublicKey Decode */
            int RsaPublicKeyDecode_fips( const byte * input,
                                         word32 * inOutIdx,
                                         RsaKey * key,
                                         word32 inSz )
            {
...
/* Rsa Check Probable Prime */
            int CheckProbablePrime_fips( const byte * p,
                                         word32 pSz,
                                         const byte * q,
                                         word32 qSz,
                                         const byte * e,
...
/* Rsa Key Gen */
            int MakeRsaKey_fips( RsaKey * key,
                                 int size,
                                 long e,
                                 WC_RNG * rng )
            {
...
/* make ECC key */
        WOLFSSL_API int ecc_make_key_fips( WC_RNG * rng,
                                           int keysize,
                                           ecc_key * key )
        {
            if( FipsAllowed() != 0 )
...
/* make ECC key extended */
        WOLFSSL_API int ecc_make_key_ex_fips( WC_RNG * rng,
                                              int keysize,
                                              ecc_key * key,
                                              int curve_id )
        {
...
/* ECC Key Import Function */
        WOLFSSL_API int ecc_import_x963_fips( const byte * in,
                                              word32 inLen,
                                              ecc_key * key )
        {
            if( FipsAllowed() != 0 )
...
/* ECDSA Signing Function */
        WOLFSSL_API int ecc_sign_hash_fips( const byte * in,
                                            word32 inlen,
                                            byte * out,
                                            word32 * outlen,
                                            WC_RNG * rng,
...
/* ECDSA Signature Verify Function */
        ...
        WOLFSSL_API int ecc_verify_hash_fips( const byte * sig,
                                              word32 siglen,
                                              const byte * hash,
                                              word32 hashlen,
                                              int * stat,
...
/* Generate a DH key pair */
        int DhGenerateKeyPair_fips( DhKey * key,
                                    WC_RNG * rng,
                                    byte * priv,
                                    word32 * privSz,
                                    byte * pub,
...
/* Check a DH public key for mathematical correctness */
        int DhCheckPubKeyEx_fips( DhKey * key,
                                  const byte * pub,
                                  word32 pubSz,
                                  const byte * prime,
                                  word32 primeSz )
...
/* Check a DH private key for mathematical correctness */
        int DhCheckPrivKeyEx_fips( DhKey * key,
                                   const byte * priv,
                                   word32 privSz,
                                   const byte * prime,
                                   word32 primeSz )
...
/* Check a DH public and private key for pair-wise consistency */
        int DhCheckKeyPair_fips( DhKey * key,
                                 const byte * pub,
                                 word32 pubSz,
                                 const byte * priv,
                                 word32 privSz )
...
/* Init RNG with Nonce */
        int InitRngNonce_fips( WC_RNG * rng,
                               byte * nonce,
                               word32 nonceSz )
        {
            int ret;
...
/* Generate block of pseudo random numbers */
        int RNG_GenerateBlock_fips( WC_RNG * rng,
                                    byte * buf,
                                    word32 bufSz )
        {
            int ret;
...
/* RNG Health Test */
        int RNG_HealthTest_fips( int reseed,
                                 const byte * entropyA,
                                 word32 entropyASz,
                                 const byte * entropyB,
                                 word32 entropyBSz,
