 ...
 */
        ...
        extern "C" {
            ...
            WOLFSSL_LOCAL void lm_copy( byte *,
                                        const byte * );
            WOLFSSL_LOCAL void lm_add( byte *,
                                       const byte *,
                                       const byte * );
            WOLFSSL_LOCAL void lm_sub( byte *,
                                       const byte *,
                                       const byte * );
            WOLFSSL_LOCAL void lm_neg( byte *,
                                       const byte * );
            WOLFSSL_LOCAL void lm_invert( byte *,
                                          const byte * );
            WOLFSSL_LOCAL void lm_mul( byte *,
                                       const byte *,
                                       const byte * );
        #endif /* if defined( CURVE25519_SMALL ) || defined( ED25519_SMALL ) */


        #if !defined( FREESCALE_LTC_ECC )
            WOLFSSL_LOCAL void fe_init( void );

            WOLFSSL_LOCAL int curve25519( byte * q,
                                          const byte * n,
                                          const byte * p );
        #endif
...
/* default to be faster but take more memory */
            ...
            WOLFSSL_LOCAL void fe_copy( fe,
                                        const fe );
            WOLFSSL_LOCAL void fe_add( fe,
                                       const fe,
                                       const fe );
            WOLFSSL_LOCAL void fe_neg( fe,
                                       const fe );
            WOLFSSL_LOCAL void fe_sub( fe,
                                       const fe,
                                       const fe );
            WOLFSSL_LOCAL void fe_invert( fe,
                                          const fe );
            WOLFSSL_LOCAL void fe_mul( fe,
                                       const fe,
                                       const fe );
 ...
 * work. */
            ...
            WOLFSSL_LOCAL void fe_tobytes( unsigned char *,
                                           const fe );
            WOLFSSL_LOCAL void fe_sq( fe,
                                      const fe );
            WOLFSSL_LOCAL void fe_sq2( fe,
                                       const fe );
            WOLFSSL_LOCAL void fe_frombytes( fe,
                                             const unsigned char * );
            WOLFSSL_LOCAL void fe_cswap( fe,
                                         fe,
                                         int );
            WOLFSSL_LOCAL void fe_mul121666( fe,
                                             fe );
            WOLFSSL_LOCAL void fe_cmov( fe,
                                        const fe,
                                        int );
            WOLFSSL_LOCAL void fe_pow22523( fe,
                                            const fe );
...
/* 64 type needed for SHA512 */
            ...
            WOLFSSL_LOCAL uint64_t load_4( const unsigned char * in );
                ...
                WOLFSSL_LOCAL void fe_ge_to_p2( fe rx,
                                                fe ry,
                                                fe rz,
                                                const fe px,
                                                const fe py,
                                                const fe pz,
                                                const fe pt );
                WOLFSSL_LOCAL void fe_ge_to_p3( fe rx,
                                                fe ry,
                                                fe rz,
                                                fe rt,
                                                const fe px,
                                                ...
                                                const fe pz,
                                                const fe pt );
                WOLFSSL_LOCAL void fe_ge_dbl( fe rx,
                                              fe ry,
                                              fe rz,
                                              fe rt,
                                              const fe px,
                                              const fe py,
                                              const fe pz );
                WOLFSSL_LOCAL void fe_ge_madd( fe rx,
                                               fe ry,
                                               fe rz,
                                               fe rt,
                                               const fe px,
                                               ...
                                               const fe qyplusx,
                                               const fe qyminusx );
                WOLFSSL_LOCAL void fe_ge_msub( fe rx,
                                               fe ry,
                                               fe rz,
                                               fe rt,
                                               const fe px,
                                               ...
                                               const fe qyplusx,
                                               const fe qyminusx );
                WOLFSSL_LOCAL void fe_ge_add( fe rx,
                                              fe ry,
                                              fe rz,
                                              fe rt,
                                              const fe px,
                                              ...
                                              const fe qyplusx,
                                              const fe qyminusx );
                WOLFSSL_LOCAL void fe_ge_sub( fe rx,
                                              fe ry,
                                              fe rz,
                                              fe rt,
                                              const fe px,
                                              ...
                                              const fe qyplusx,
                                              const fe qyminusx );
                WOLFSSL_LOCAL void fe_cmov_table( fe * r,
                                                  fe * base,
                                                  signed char b );
 ...
 * Based on Daniel Beer's public domain work. */
        ...
        WOLFSSL_LOCAL void fe_load( byte * x,
                                    word32 c );
        WOLFSSL_LOCAL void fe_normalize( byte * x );
        WOLFSSL_LOCAL void fe_inv__distinct( byte * r,
                                             const byte * x );
 ...
 * undefined behavior.
 */
        WOLFSSL_LOCAL void fe_select( byte * dst,
                                      const byte * zero,
                                      const byte * one,
                                      byte condition );
 ...
 */
        WOLFSSL_LOCAL void fe_mul_c( byte * r,
                                     const byte * a,
                                     word32 b );
        WOLFSSL_LOCAL void fe_mul__distinct( byte * r,
                                             const byte * a,
                                             const byte * b );
 ...
 */
        WOLFSSL_LOCAL void fe_sqrt( byte * r,
                                    const byte * x );
 ...
 * undefined behavior.
 */
        WOLFSSL_LOCAL void fprime_select( byte * dst,
                                          const byte * zero,
                                          const byte * one,
                                          byte condition );
        WOLFSSL_LOCAL void fprime_add( byte * r,
                                       const byte * a,
                                       const byte * modulus );
        WOLFSSL_LOCAL void fprime_sub( byte * r,
                                       const byte * a,
                                       const byte * modulus );
        WOLFSSL_LOCAL void fprime_mul( byte * r,
                                       const byte * a,
                                       const byte * b,
                                       const byte * modulus );
        WOLFSSL_LOCAL void fprime_copy( byte * x,
                                        const byte * a );
