...
/* Some helper functions, not necessarily useful */
    ...
    static WC_INLINE int blake2s_increment_counter( blake2s_state * S,
                                                    const word32
                                                    inc )
    {
        S->t[ 0 ] += inc;
...
/* init xors IV with input parameter block */
                      ...
                      const byte outlen )
    {
        ...
        #else  /* ifdef WOLFSSL_BLAKE2S_INIT_EACH_FIELD */
            XMEMSET( P, 0, sizeof( *P ) );
            P->digest_length = outlen;
            P->fanout = 1;
            P->depth = 1;
        #endif /* ifdef WOLFSSL_BLAKE2S_INIT_EACH_FIELD */
...
/* Blake2s Update */
    int wc_Blake2sUpdate( Blake2s * b2s,
                          const byte * data,
                          word32 sz )
    {
...
/* Blake2s Final, if pass in zero size we use init digestSz */
    int wc_Blake2sFinal( Blake2s * b2s,
                         byte * final,
                         word32 requestSz )
    {
