...
#define __MIDI_CLASS_HOST_H__
	...
	/* Public Interface - May be used in end-application: */
		...
		/* Function Prototypes: */
			/** @brief Host interface configuration routine, to configure a given MIDI host interface instance using the Configuration
			 ...
			 *  the Addressed state.
			 *
			 *  @param MIDIInterfaceInfo      : Pointer to a structure containing an MIDI Class host configuration and state.
			...
			/** @brief General management task for a given MIDI host class interface, required for the correct operation of the interface. This should
			...
			/** @brief Sends a MIDI event packet to the device. If no device is connected, the event packet is discarded.
			 *
			...
			/** @brief Flushes the MIDI send buffer, sending any queued MIDI events to the device. This should be called to override the
			...
			/** @brief Receives a MIDI event packet from the device.
			 *
			 *  @pre This function must only be called when the Host state machine is in the @ref HOST_STATE_Configured state or the
			 *       call will fail.
			 *
