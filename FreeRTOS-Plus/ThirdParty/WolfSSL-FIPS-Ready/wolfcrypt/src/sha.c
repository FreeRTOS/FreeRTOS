...
/* Supports CubeMX HAL or Standard Peripheral Library */
                ...
                static int Transform( wc_Sha * sha,
                                      const byte * data )
                {
                ...
                static int Transform_Len( wc_Sha * sha,
                                          const byte * data,
                                          word32 len )
                {
                    int ret = wolfSSL_CryptHwMutexLock();
...
/* implemented in wolfcrypt/src/port/Renesas/renesas_tsip_sha.c */
            ...
            #else  /* if defined( WOLFSSL_PIC32MZ_HASH ) */
...
/* Software implementation */
                ...
                static WC_INLINE void AddLength( wc_Sha * sha,
                                                 word32 len )
                {
...
/* (R0+R1), R2, R3, R4 are the different operations used in SHA1 */
    ...
    ( z ) += f1( ( w ), ( x ), ( y ) ) + blk0( ( i ) ) + 0x5A827999 + \
             rotlFixed( ( v ), 5 ); ( w ) = rotlFixed( ( w ), 30 );
                    #define R1( v, w, x, y, z, i )                    \
    ( z ) += f1( ( w ), ( x ), ( y ) ) + blk1( ( i ) ) + 0x5A827999 + \
             rotlFixed( ( v ), 5 ); ( w ) = rotlFixed( ( w ), 30 );
                    #define R2( v, w, x, y, z, i )                    \
    ( z ) += f2( ( w ), ( x ), ( y ) ) + blk1( ( i ) ) + 0x6ED9EBA1 + \
             rotlFixed( ( v ), 5 ); ( w ) = rotlFixed( ( w ), 30 );
                    #define R3( v, w, x, y, z, i )                    \
    ( z ) += f3( ( w ), ( x ), ( y ) ) + blk1( ( i ) ) + 0x8F1BBCDC + \
             rotlFixed( ( v ), 5 ); ( w ) = rotlFixed( ( w ), 30 );
                    #define R4( v, w, x, y, z, i )                    \
    ( z ) += f4( ( w ), ( x ), ( y ) ) + blk1( ( i ) ) + 0xCA62C1D6 + \
             rotlFixed( ( v ), 5 ); ( w ) = rotlFixed( ( w ), 30 );
