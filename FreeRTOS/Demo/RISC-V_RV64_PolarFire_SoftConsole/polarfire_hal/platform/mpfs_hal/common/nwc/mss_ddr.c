...
/* Use to record instance of errors during calibration */
    static uint32_t ddr_error_count;
        ...
        #define TOTAL_SWEEPS                                              \
    ( MAX_NUMBER_DPC_H_GEN_SWEEPS * MAX_NUMBER_DPC_H_GEN_SWEEPS *         \
...
/*static uint8_t mode_register_write(uint32_t MR_ADDR, uint32_t MR_DATA);*/
    static uint8_t MTC_test( uint8_t mask,
                             uint64_t start_address,
                             uint32_t size,
                             MTC_PATTERN pattern,
                             MTC_ADD_PATTERN add_pattern,
                             uint32_t * error );
    #ifdef VREFDQ_CALIB
    ...
    #endif
    #ifdef DDR_SANITY_CHECKS_EN
        static uint8_t rw_sanity_chk( uint64_t * address,
                                      uint32_t count );
        static uint8_t mtc_sanity_check( uint64_t start_address );
    #endif
    #ifdef SET_VREF_LPDDR4_MODE_REGS
        static uint8_t mode_register_write( uint32_t MR_ADDR,
                                            uint32_t MR_DATA );
    #endif
    #ifdef DDR_SANITY_CHECKS_EN
    ...
    static void config_ddr_io_pull_up_downs_rpc_bits( DDR_TYPE ddr_type );
        ...
        static uint8_t ddr_manual_addcmd_refclk_offset( DDR_TYPE ddr_type,
...
/* To verify if separate reset required for DDR4 - believe it is not */
                    ...
                    /* reset pin is bit [1] */
                    CFG_DDR_SGMII_PHY->training_reset.training_reset = 0x00000002U;
                #endif /* ifndef SPECIAL_TRAINIG_RESET */
            ...
            case DDR_TRAINING_IP_SM_START:
               {
                   ...
                   CFG_DDR_SGMII_PHY->training_reset.training_reset = 0x00000000U;
                       ...
                       delay( 100 );
                   #endif /* ifdef IP_SM_START_TRAINING_PAUSE */
               }
                {
            ...
            case DDR_VERIFY_PATTERN_IN_CACHE:
                ...
                else
                {
                            ...
                            ddr_training_state = DDR_TRAINING_FAIL;
                        #endif /* ifdef NOT_A_FULL_RETRAIN */
                    #else /* (TUNE_RPC_166_VALUE == 0) */
                        ddr_training_state = DDR_TRAINING_FAIL;
                    #endif /* if ( TUNE_RPC_166_VALUE == 1 ) */
                }
         ...
         */
        {
            switch( ddr_type )
            {
                ...
                case LPDDR4:
                    ...
                    #endif /* ifndef LIBERO_SETTING_RPC_EN_ADDCMD0_OVRT9 */
 ...
 */
    ...
    #endif /* ifdef DDR_SANITY_CHECKS_EN */
 ...
 */
        ...
        static uint8_t read_back_sanity_check( uint64_t * address,
                                               uint32_t count )
        {
            volatile uint64_t * DDR_word_ptr;
            uint8_t error = 0U;
 ...
 */
    #ifdef SW_CONFIG_LPDDR_WR_CALIB_FN
        static void increment_dq( uint8_t lane,
                                  uint32_t move_count )
        {
            /*set dyn_ovr_dlycnt_dq_move* = 0 */
            if( lane < 4U )
    ...
    #endif /* ifdef SW_CONFIG_LPDDR_WR_CALIB_FN */
 ...
 * @return
 */
    static uint8_t write_calibration_using_mtc( uint8_t number_of_lanes_to_calibrate )
    {
        uint8_t laneToTest;
        uint32_t result = 0U;
        uint32_t cal_data;
 ...
 */
    ...
    #endif /* ifdef SET_VREF_LPDDR4_MODE_REGS */
 ...
 */
    static void config_ddr_io_pull_up_downs_rpc_bits( DDR_TYPE ddr_type )
    {
        if( ddr_type == LPDDR4 ) /* we will add other variants here once verified */
        {
            ...
            #endif /* ifdef LIBERO_SETTING_RPC_EN_ADDCMD0_OVRT9 */
        }
    }
 ...
 */
        ...
        static uint8_t get_best_sweep( sweep_index * good_index )
        {
                ...
                return( 0U );
            #endif /* ifdef EXTRACT_SWEEP_RESULT */
        }
    #endif /* SWEEP_ENABLED */
 ...
 *  @endcode
 */
        uint8_t MSS_DDR_status( uint8_t option,
 ...
 *    @endcode
 */
        uint8_t MSS_DDR_user_commands( uint8_t command,
                                       uint32_t * extra_command_data,
                                       uint32_t * return_data, \
 ...
 */
    ...
    #endif /* ifdef MANUAL_ADDCMD_TRAINIG */
