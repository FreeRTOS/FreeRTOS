...
/* function converts int hash type to enum */
    enum wc_HashType wc_HashTypeConvert( int hashType )
    {
        ...
        #else  /* if defined( HAVE_FIPS ) || defined( HAVE_SELFTEST ) */
        ...
        #endif /* if defined( HAVE_FIPS ) || defined( HAVE_SELFTEST ) */
        return eHashType;
    }
...
/* Generic Hashing Wrapper */
    ...
    int wc_HashInit_ex( wc_HashAlg * hash,
    ...
    int wc_HashInit( wc_HashAlg * hash,
                     enum wc_HashType type )
    {
        return wc_HashInit_ex( hash, type, NULL, INVALID_DEVID );
    }

    int wc_HashUpdate( wc_HashAlg * hash,
                       enum wc_HashType type,
                       const byte * data,
                       word32 dataSz )
    {
    ...
    int wc_HashFinal( wc_HashAlg * hash,
                      enum wc_HashType type,
                      byte * out )
    {
        int ret = HASH_TYPE_E; /* Default to hash type error */
    ...
    int wc_HashFree( wc_HashAlg * hash,
                     enum wc_HashType type )
    {
        int ret = HASH_TYPE_E; /* Default to hash type error */
        ...
        int wc_HashSetFlags( wc_HashAlg * hash,
                             enum wc_HashType type,
                             word32 flags )
        {
            int ret = HASH_TYPE_E; /* Default to hash type error */
        ...
        int wc_HashGetFlags( wc_HashAlg * hash,
                             enum wc_HashType type,
                             word32 * flags )
        {
            int ret = HASH_TYPE_E; /* Default to hash type error */
