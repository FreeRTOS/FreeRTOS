...
/* THREADING/MUTEX SECTION */
            ...
            #include "system/xtl.h"
                ...
                #include <windows.h>
            #endif /* WOLFSSL_SGX */
        #endif /* ifdef WOLFSSL_GAME_BUILD */
...
/* dynamically set which mutex to use. unlock / lock is controlled by flag */
        typedef void (mutex_cb)( int flag,
                                 int type,
                                 const char * file,
                                 int line );

        WOLFSSL_API int wc_LockMutex_ex( int flag,
                                         int type,
                                         const char * file,
                                         int line );
        WOLFSSL_API int wc_SetMutexCb( mutex_cb * cb );
    #endif /* if defined( OPENSSL_EXTRA ) || defined( HAVE_WEBSERVER ) */
...
/* filesystem abstraction layer, used by ssl.c */
            ...
            int ebsnet_fseek( int a,
                              long b,
                              int c );      /* Not prototyped in vfile.h per
                                             * EBSnet feedback */
            ...
            XFILE z_fs_open( const char * filename,
                             const char * perm );
        ...
        #else  /* if defined( EBSNET ) */
            /* stdio, default case */
            #include <stdio.h>
            #define XFILE    FILE *
            #if defined( WOLFSSL_MDK_ARM )
                extern FILE * wolfSSL_fopen( const char * name,
                                             const char * mode );
        ...
        WOLFSSL_LOCAL int wc_FileLoad( const char * fname,
                                       unsigned char ** buf,
                                       size_t * bufLen,
                                       void * heap );
        ...
        !defined( WOLFSSL_NUCLEUS_1_2 )
            ...
            WOLFSSL_API int wc_ReadDirFirst( ReadDirCtx * ctx,
                                             const char * path,
                                             char ** name );
            WOLFSSL_API int wc_ReadDirNext( ReadDirCtx * ctx,
                                            const char * path,
                                            char ** name );
...
/* callbacks for setting handle */
        typedef int (* wolfSSL_DSP_Handle_cb)( remote_handle64 * handle,
                                               int finished,
                                               void * ctx );
