...
#define IS_SPI_CONFIG_IT( IT )                         \
    ( ( IT == SPI_IT_TXE ) || ( IT == SPI_IT_RXNE ) || \
      ( IT == SPI_IT_ERR ) )
...
#define IS_SPI_GET_FLAG( FLAG )                                   \
    ( ( FLAG == SPI_FLAG_BSY ) || ( FLAG == SPI_FLAG_OVR ) ||     \
...
void SPI_CalculateCRC( SPI_TypeDef * SPIx,
                       FunctionalState NewState );
u16 SPI_GetCRC( SPI_TypeDef * SPIx,
                u8 SPI_CRC );
